...
public class TutorialObjectiveMouseParentWidget : Widget
{
	...
	protected override void OnUpdate(float dt)
	{
		...
		_animationsSet = true;
		BrushAnimation animation4 = MouseLeftClickWidget.Brush.GetAnimation("BlinkAnimation");
		foreach (BrushAnimation animation3 in MouseLeftClickWidget.Brush.GetAnimations())
		{
			if (animation3 == animation4)
			{
				continue;
			}
			foreach (BrushLayerAnimation layerAnimation in animation4.GetLayerAnimations())
			{
		...
		foreach (BrushAnimation animation2 in MouseRightClickWidget.Brush.GetAnimations())
		{
			if (animation2 == animation4)
			{
				continue;
			}
			foreach (BrushLayerAnimation layerAnimation2 in animation4.GetLayerAnimations())
			{
		...
		foreach (BrushAnimation animation in MouseMiddleClickWidget.Brush.GetAnimations())
		{
			if (animation == animation4)
			{
				continue;
			}
			foreach (BrushLayerAnimation layerAnimation3 in animation4.GetLayerAnimations())
			{
