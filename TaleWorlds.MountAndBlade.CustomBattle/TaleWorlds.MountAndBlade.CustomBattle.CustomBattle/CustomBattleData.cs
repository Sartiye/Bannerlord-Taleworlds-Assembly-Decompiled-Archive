...
public struct CustomBattleData
{
	...
	public static IEnumerable<Tuple<string, string>> GameTypes
	{
		get
		{
			yield return new Tuple<string, string>(GameTexts.FindText("str_battle").ToString(), "Battle");
			if (!Module.CurrentModule.IsOnlyCoreContentEnabled)
			{
				yield return new Tuple<string, string>(new TextObject("{=Ua6CNLBZ}Village").ToString(), "Village");
				yield return new Tuple<string, string>(GameTexts.FindText("str_siege").ToString(), "Siege");
			}
		}
	}
