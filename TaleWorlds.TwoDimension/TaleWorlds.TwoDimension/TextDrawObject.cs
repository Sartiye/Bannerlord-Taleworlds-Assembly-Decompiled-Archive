...
public struct TextDrawObject : IDrawObject
{
	...
	{
		TextDrawObject result = default(TextDrawObject);
	...
	public static TextDrawObject Create(float[] vertices, float[] uvs, uint[] indices, float text_MeshWidth, float text_MeshHeight, in Rectangle2D rectangle)
	{
		TextDrawObject result = default(TextDrawObject);
		result.IsValid = true;
		result.Text_Vertices = vertices;
		result.Text_TextureCoordinates = uvs;
		result.Text_Indices = indices;
		result.Text_MeshWidth = text_MeshWidth;
		result.Text_MeshHeight = text_MeshHeight;
