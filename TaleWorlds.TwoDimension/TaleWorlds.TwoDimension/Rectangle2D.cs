...
public struct Rectangle2D
{
	private struct RectangleRenderProperties
	{
		...
		{
			RectangleRenderProperties result = default(RectangleRenderProperties);
			result.ScaleMultiplier = Vector2.One;
			result.PositionOffsetPixel = Vector2.Zero;
			result.RotationOffset = 0f;
			return result;
		}
	...
	private static class RectangleHelper
	{
		...
		public static MatrixFrame CreateMatrixFrame(float posX, float posY, float pivotX, float pivotY, float scaleX, float scaleY, float rotation)
		{
			...
			o += rot.TransformToParent(in v);
			Vec3 scaleAmountXYZ = new Vec3(scaleX, scaleY, 1f);
		...
		private static bool AreLinesIntersecting(in Vector2 line1Start, in Vector2 line1End, in Vector2 line2Start, in Vector2 line2End)
		{
			...
			Vector2 p3 = vector2 - vector;
			float num = GetTwoDimensionalCrossProduct(in p3, in p2) / twoDimensionalCrossProduct;
			p3 = vector2 - vector;
			float num2 = GetTwoDimensionalCrossProduct(in p3, in p) / twoDimensionalCrossProduct;
	...
	{
		Rectangle2D result = default(Rectangle2D);
	...
	public static Rectangle2D Create()
	{
		Rectangle2D result = default(Rectangle2D);
		result.IsValid = true;
		result.LocalScale = new Vector2(10f, 10f);
		result._renderProperties = RectangleRenderProperties.CreateEmpty();
		result._cachedOrthonormalMatrix = MatrixFrame.Identity;
		result._cachedMatrixFrame = MatrixFrame.Identity;
		result._cachedVisualMatrixFrame = MatrixFrame.Identity;
		return result;
	}
	...
	{
		ref MatrixFrame cachedOrthonormalMatrix = ref _cachedOrthonormalMatrix;
		Vec3 v = new Vec3(screenPosition.X, screenPosition.Y);
		Vec3 vec = cachedOrthonormalMatrix.TransformToLocal(in v);
	...
	{
		ref MatrixFrame cachedOrthonormalMatrix = ref _cachedOrthonormalMatrix;
		Vec3 v = new Vec3(localPosition.X, localPosition.Y);
		Vec3 vec = cachedOrthonormalMatrix.TransformToParent(in v);
		return new Vector2(vec.x, vec.y);
	}
}
