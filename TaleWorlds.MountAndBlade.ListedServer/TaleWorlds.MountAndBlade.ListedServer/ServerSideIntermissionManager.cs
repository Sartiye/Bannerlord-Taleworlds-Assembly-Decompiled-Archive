		...
		if (_listedServer.IsRegistered && !_listedServer.IsPlaying)
		{
			...
			if (flag)
			{
				if (GameStateManager.Current.ActiveState is InitialListedGameServerState)
				{
					...
					string strValue5 = MultiplayerOptions.OptionType.SpectatorPassword.GetStrValue();
					...
					string value3 = ((!string.IsNullOrEmpty(strValue5)) ? Common.CalculateMD5Hash(strValue5) : null);
	...
	private async Task StartMissionAux()
	{
		if (_listedServer.IsRegistered && _listedServer.IsPlaying)
		{
			if (GameStateManager.Current.ActiveState is IIntermissionState)
			{
				...
				{
					Debug.FailedAssert("[DEBUG]Invalid multiplayer game type.", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade.ListedServer\\ServerSideIntermissionManager.cs", "StartMissionAux", 718);
				}
