...
internal class DWAKdTree
{
	...
	internal void BuildAgentTree()
	{
		if (_agents == null || _agents.Length != _simulator.NumAgents)
		{
			...
			int num = 0;
			for (int i = 0; i < _simulator.AgentsIncludingRemoved.Count; i++)
			{
				DWAAgent dWAAgent = _simulator.AgentsIncludingRemoved[i];
				if (dWAAgent != null)
				{
	...
	internal void BuildObstacleTree()
	{
		...
		{
			list.Add(_simulator.Obstacles[i]);
		}
	...
	private DWAObstacleTreeNode BuildObstacleTreeRecursive(IList<DWAObstacleVertex> obstacles)
	{
		...
		for (int i = 0; i < obstacles.Count; i++)
		{
			...
			for (int j = 0; j < obstacles.Count; j++)
			{
				if (i != j)
				{
					...
					Vec2 lineSegmentBegin = dWAObstacleVertex.Point;
					Vec2 lineSegmentEnd = next.Point;
					Vec2 point = dWAObstacleVertex2.Point;
					float signedDistanceOfPointToLineSegment = MBMath.GetSignedDistanceOfPointToLineSegment(in lineSegmentBegin, in lineSegmentEnd, in point);
					lineSegmentBegin = dWAObstacleVertex.Point;
					lineSegmentEnd = next.Point;
					point = next2.Point;
					float signedDistanceOfPointToLineSegment2 = MBMath.GetSignedDistanceOfPointToLineSegment(in lineSegmentBegin, in lineSegmentEnd, in point);
		...
		for (int m = 0; m < obstacles.Count; m++)
		{
			...
			Vec2 lineSegmentBegin = dWAObstacleVertex3.Point;
			Vec2 lineSegmentEnd = next3.Point;
			Vec2 point = dWAObstacleVertex4.Point;
			float signedDistanceOfPointToLineSegment3 = MBMath.GetSignedDistanceOfPointToLineSegment(in lineSegmentBegin, in lineSegmentEnd, in point);
			lineSegmentBegin = dWAObstacleVertex3.Point;
			lineSegmentEnd = next3.Point;
			point = next4.Point;
			float signedDistanceOfPointToLineSegment4 = MBMath.GetSignedDistanceOfPointToLineSegment(in lineSegmentBegin, in lineSegmentEnd, in point);
			...
			lineSegmentBegin = next3.Point - dWAObstacleVertex3.Point;
			lineSegmentEnd = dWAObstacleVertex4.Point - dWAObstacleVertex3.Point;
			float num9 = Vec2.Determinant(in lineSegmentBegin, in lineSegmentEnd);
			point = next3.Point - dWAObstacleVertex3.Point;
			Vec2 vec = dWAObstacleVertex4.Point - next4.Point;
			float num10 = num9 / Vec2.Determinant(in point, in vec);
			Vec2 point2 = dWAObstacleVertex4.Point + num10 * (next4.Point - dWAObstacleVertex4.Point);
			...
			dWAObstacleVertex5.Point = point2;
	...
	private void QueryObstacleTreeRecursive(DWAAgent agent, ref float rangeSq, DWAObstacleTreeNode node)
	{
		...
		Vec2 lineSegmentEnd = next.Point;
		float signedDistanceOfPointToLineSegment = MBMath.GetSignedDistanceOfPointToLineSegment(in lineSegmentBegin, in lineSegmentEnd, in agent.State.Position);
		QueryObstacleTreeRecursive(agent, ref rangeSq, (signedDistanceOfPointToLineSegment >= 0f) ? node.Left : node.Right);
		if (signedDistanceOfPointToLineSegment * signedDistanceOfPointToLineSegment / (next.Point - obstacle.Point).LengthSquared < rangeSq)
		{
	...
	private bool QueryVisibilityRecursive(in Vec2 q1, in Vec2 q2, float radius, DWAObstacleTreeNode node)
	{
		...
		Vec2 lineSegmentBegin = obstacle.Point;
		Vec2 lineSegmentEnd = next.Point;
		float signedDistanceOfPointToLineSegment = MBMath.GetSignedDistanceOfPointToLineSegment(in lineSegmentBegin, in lineSegmentEnd, in q1);
		lineSegmentBegin = obstacle.Point;
		lineSegmentEnd = next.Point;
		float signedDistanceOfPointToLineSegment2 = MBMath.GetSignedDistanceOfPointToLineSegment(in lineSegmentBegin, in lineSegmentEnd, in q2);
		float num = 1f / (next.Point - obstacle.Point).LengthSquared;
		...
		lineSegmentBegin = obstacle.Point;
		float signedDistanceOfPointToLineSegment3 = MBMath.GetSignedDistanceOfPointToLineSegment(in q1, in q2, in lineSegmentBegin);
		lineSegmentBegin = next.Point;
		float signedDistanceOfPointToLineSegment4 = MBMath.GetSignedDistanceOfPointToLineSegment(in q1, in q2, in lineSegmentBegin);
		float num5 = 1f / (q2 - q1).LengthSquared;
