	...
	private void CalculateAndSetShipOrders()
	{
		...
		case ShipDefenseState.StandInLine:
		{
			...
			if (_isAnchor)
			{
				...
				{
					direction = vec;
					position = globalFrame.origin.AsVec2 + direction * 15f;
				}
			}
			else
			{
				MatrixFrame globalFrame2 = _allyShip.GlobalFrame;
				(globalFrame.origin - globalFrame2.origin).Normalize();
				Vec2 vec2 = (_actualRightSide ? (_navalTeamAI.UseSpawnPathApproachPosition ? _allyShip.GlobalFrame.rotation.f.AsVec2.RightVec() : vec.RightVec()) : (_navalTeamAI.UseSpawnPathApproachPosition ? _allyShip.GlobalFrame.rotation.f.AsVec2.LeftVec() : vec.LeftVec()));
				position = _allyShip.GlobalFrame.origin.AsVec2 + vec * 10f + vec2 * 30f;
				float num = (position - _formationMainShip.GlobalFrame.origin.AsVec2).DotProduct(vec);
				if (num < 0f)
				{
					position += num * vec;
				}
