...
public class NavalStorylineThirdActFirstQuestBehavior : CampaignBehaviorBase
{
	...
	private void AddGunnarInitialDialogFlow()
	{
		Campaign.Current.ConversationManager.AddDialogFlow(DialogFlow.CreateDialogFlow("start", 1200).NpcLine("{=HTEIIesY}Greetings. Listen… When we sailed with Purig, I was hoping that he would help me fight the Sea Hounds near Hvalvik. His betrayal of course has cost us time, but I think that plan is still a good one.").Condition(() => IsQuest1ReadyToStart() && !NavalStorylineData.IsTutorialSkipped() && !_isIntroGiven)
			...
			.NpcLine("{=Tn5mFdcU}Such a prize would be a great boon to the Sea Hounds. I propose that we deny it to them. We can sail to Hvalvik, meet this merchant, and escort them south, sinking or taking any Sea Hounds we encounter.")
			.NpcLine("{=DRRUMKFN}Our longship is ready. If you can join me, then we should set out as soon as you are ready.")
			.NpcLine("{=xngacVnQ}One thing - it is hard to revictual at sea, so do make sure we have plenty of supplies with us to go to Hvalvik and back. Twenty loads of grain and meat, or the equivalent, should be sufficient for our voyage.")
			...
			.NpcLine("{=bhUo9L89}Splendid. The tide and winds are with us. Let us go forth!")
			.Consequence(delegate
			{
			...
			.NpcLine("{=mw07yfTt}Very well. We can wait here for a bit longer for you.")
			.CloseDialog()
		...
		Campaign.Current.ConversationManager.AddDialogFlow(DialogFlow.CreateDialogFlow("start", 1200).NpcLine("{=NSHm5s2u}{PLAYER.NAME}... It's good to see you again! Have you reconsidered joining me in my little feud? I cannot promise you that we will find your sister, but I believe the odds have increased.").Condition(delegate
		{
			...
			.NpcLine("{=zYguiNhG}Anyway, I had originally wanted to join up with a merchant returning to Vlandia from Hvalvik, and I think that plan is still a good one. His company has spent the last months hunting and whaling in the far north, and their ship is laden with valuables. I am certain that the Sea Hounds will be unable to resist such a tempting target.")
			.Consequence(delegate
			{
				_isIntroGiven = true;
			})
			...
			.GotoDialogState("gunnar_ransom_sister")
			.EndPlayerOptions());
		Campaign.Current.ConversationManager.AddDialogFlow(DialogFlow.CreateDialogFlow("start", 1500).NpcLine("{=yJIP3tpk}Are we ready to sail to Hvalvik to escort those Vlandian merchants? They will wait as long as they can, but they cannot wait forever.").Condition(() => IsQuest1ReadyToStart() && _isIntroGiven)
			.BeginPlayerOptions()
