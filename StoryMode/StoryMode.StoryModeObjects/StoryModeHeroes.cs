...
public class StoryModeHeroes
{
	...
	private void RegisterAll()
	{
		...
		if (HeroCreator.CreateBasicHero("main_hero_mother", @object, out _mainHeroMother, isAlive: false))
		{
			_mainHeroMother.Clan = clan;
			HeroHelper.GetRandomDeathDayAndBirthDay((int)@object.Age, out var birthday, out var deathday);
			_mainHeroMother.SetBirthDay(birthday);
			_mainHeroMother.SetDeathDay(deathday);
		}
		if (HeroCreator.CreateBasicHero("main_hero_father", object2, out _mainHeroFather, isAlive: false))
		{
			_mainHeroFather.Clan = clan;
			HeroHelper.GetRandomDeathDayAndBirthDay((int)object2.Age, out var birthday2, out var deathday2);
