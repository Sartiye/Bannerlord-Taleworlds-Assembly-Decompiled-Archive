...
public class SecondPhase
{
	...
	[SaveableProperty(6)]
	public Clan ConspiracyClan { get; private set; }
	...
	protected virtual void AutoGeneratedInstanceCollectObjects(List<object> collectedObjects)
	{
		...
		collectedObjects.Add(ConspiracyClan);
	}
	...
	internal static object AutoGeneratedGetMemberValueConspiracyClan(object o)
	{
		return ((SecondPhase)o).ConspiracyClan;
	}
	...
	public void CreateConspiracyClan()
	{
		ConspiracyClan = new Clan();
		ConspiracyClan.StringId = Campaign.Current.CampaignObjectManager.FindNextUniqueStringId<Clan>("conspiracy_clan");
		TextObject textObject = (StoryModeManager.Current.MainStoryLine.IsOnImperialQuestLine ? new TextObject("{=DZKPjzGb}Valdros") : new TextObject("{=pHBad5rE}Zarvethi"));
		ConspiracyClan.ChangeClanName(textObject, textObject);
		string objectName = (StoryModeManager.Current.MainStoryLine.IsOnImperialQuestLine ? "battania" : "empire");
		ConspiracyClan.Culture = MBObjectManager.Instance.GetObject<CultureObject>(objectName);
		string message = (StoryModeManager.Current.MainStoryLine.IsOnImperialQuestLine ? "11.67.166.1528.1528.764.764.1.0.0.423.17.21.623.550.764.764.0.0.0.103.5.171.315.274.746.780.0.0.0" : "11.149.166.1528.1528.764.767.1.0.0.302.3.171.483.483.785.767.0.0.360.307.3.171.483.483.613.764.0.0.0.305.3.171.483.483.905.779.0.0.0");
		uint color = (StoryModeManager.Current.MainStoryLine.IsOnImperialQuestLine ? 4284762484u : 4280360735u);
		uint color2 = (StoryModeManager.Current.MainStoryLine.IsOnImperialQuestLine ? 4293842576u : 4290048241u);
		Banner banner = new Banner();
		banner.Deserialize(message);
		ConspiracyClan.Banner = banner;
		ConspiracyClan.Color = color;
		ConspiracyClan.Color2 = color2;
		foreach (Kingdom item in Kingdom.All)
		{
			if (StoryModeManager.Current.MainStoryLine.IsOnImperialQuestLine == StoryModeData.IsKingdomImperial(item))
			{
				DeclareWarAction.ApplyByQuest(ConspiracyClan, item);
			}
		}
	}
}
