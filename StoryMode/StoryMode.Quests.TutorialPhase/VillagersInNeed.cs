...
public class VillagersInNeed : StoryModeQuestBase
{
	...
	{
		if (Settlement.CurrentSettlement == _village && args.MenuContext.GameMenu.StringId == "village")
		{
			if (_startVillaMission)
			{
				StartVillaMission();
			}
			else if (!_talkedToVillagers && !_firstConversationWithVillagerOpened && !_startVillaMission)
			{
				OpenConversationWithVillager();
			}
			else if (_rescuedHeadman)
			{
	...
	private bool village_talk_to_villager_on_condition(MenuCallbackArgs args)
	{
		...
		bool result = Settlement.CurrentSettlement == _village && _talkedToVillagers;
		if (CampaignTime.Now.IsDayTime)
		{
			args.IsEnabled = false;
			args.Tooltip = new TextObject("{=8RJpNhWh}Come back at night time to talk to the villager.");
		}
		return result;
	}
	...
	{
		if (!CampaignTime.Now.IsDayTime && !_talkedToVillagers && settlement == _village && party == MobileParty.MainParty)
		{
			OpenConversationWithVillager();
		}
	}
	...
	private void OpenConversationWithVillager()
	{
		...
		_firstConversationWithVillagerOpened = true;
	}
	...
	protected override void SetDialogs()
	{
		Campaign.Current.ConversationManager.AddDialogFlow(DialogFlow.CreateDialogFlow("start", 1000010).NpcLine(new TextObject("{=!}{VILLAGER_DIALOGUE_1}")).Condition(talk_to_villagers_on_condition)
			...
			.EndPlayerOptions()
			.NpcLine(new TextObject("{=4YDqx6uZ}Thank you, my {?PLAYER.GENDER}lady{?}lord{\\?}. I'll take you to the villa so that we arrive at night. I doubt they'll be on their guard given all the wine they have stolen from us."), null, null, token2)
			...
			.Condition(() => CampaignTime.Now.IsNightTime)
			.CloseDialog()
			.PlayerOption(new TextObject("{=OWUiQbHx}I will come back at night."))
			.Condition(() => !CampaignTime.Now.IsNightTime)
			.CloseDialog()
