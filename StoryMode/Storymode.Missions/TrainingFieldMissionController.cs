...
public class TrainingFieldMissionController : MissionLogic
{
	...
	{
		if (worldPosition.AsVec2.NearlyEquals(_rangedTargetPosition.AsVec2, 0.001f))
		{
			Vec3 groundVec = worldPosition.GetGroundVec3();
			Vec3 v = _rangedTargetPosition.GetGroundVec3();
			if (groundVec.NearlyEquals(in v, 0.001f) && rotation.NearlyEquals(in _rangedTargetRotation))
			{
				return;
			}
		}
	...
	private void UpdateBowTraining()
	{
		...
		if (!_targetPositionSet)
		{
			...
			Vec2 targetPosition = _bowNpc.Position.AsVec2;
			bowNpc.SetTargetPositionAndDirection(in targetPosition, in _rangedTargetRotation);
