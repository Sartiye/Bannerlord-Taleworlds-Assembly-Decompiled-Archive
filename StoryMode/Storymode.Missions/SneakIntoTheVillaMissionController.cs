...
public class SneakIntoTheVillaMissionController : MissionLogic
{
	...
	private void SpawnStealthAgents()
	{
		...
		foreach (GameEntity item in entities)
		{
			foreach (GameEntity child in item.GetChildren())
			{
				...
				{
					Agent agent = SpawnAgent(@object, child, base.Mission.PlayerEnemyTeam);
					...
					AnimationSystemData animationSystemData = agent.Monster.FillAnimationSystemData(actionSet, @object.GetStepSize(), hasClippingPlane: false);
	...
	private Agent SpawnAgent(CharacterObject character, GameEntity spawnPoint, Team team)
	{
		...
		AgentBuildData agentBuildData = new AgentBuildData(character).NoHorses(noHorses: true).InitialPosition(in globalFrame.origin);
		Vec2 direction = globalFrame.rotation.f.AsVec2.Normalized();
		AgentBuildData agentBuildData2 = agentBuildData.InitialDirection(in direction).CivilianEquipment(civilianEquipment: true).Team(team)
			.TroopOrigin(new SimpleAgentOrigin(character));
		return Mission.Current.SpawnAgent(agentBuildData2);
	}
}
