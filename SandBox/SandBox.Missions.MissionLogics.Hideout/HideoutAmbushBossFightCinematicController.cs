...
public class HideoutAmbushBossFightCinematicController : MissionLogic
{
	...
	public void GetAllyFrames(out List<MatrixFrame> initialFrames, out List<MatrixFrame> targetFrames, MatrixFrame initialPlayerFrame, MatrixFrame targetPlayerFrame, int agentCount, float agentOffsetAngle)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			Vec3 o = new Vec3(initialPlayerFrame.origin.x, initialPlayerFrame.origin.y - 1.3f * (float)num, initialPlayerFrame.origin.z);
			array[num2] = new MatrixFrame(in rotation, in o);
		}
		for (int j = 0; j < array.Length; j++)
		{
			...
			for (int k = 0; k < num4; k++)
			{
				...
				Vec3 o = new Vec3(array[j].origin.x - 1f * (float)(k + 1), array[j].origin.y, array[j].origin.z);
				obj.Add(new MatrixFrame(in rotation2, in o));
			}
			for (int l = 0; l < num5; l++)
			{
				List<MatrixFrame> obj2 = initialFrames;
				ref Mat3 rotation3 = ref array[j].rotation;
				Vec3 o = new Vec3(array[j].origin.x + 1f * (float)(l + 1), array[j].origin.y, array[j].origin.z);
				obj2.Add(new MatrixFrame(in rotation3, in o));
			}
		}
		foreach (MatrixFrame initialFrame in initialFrames)
		{
			...
			Vec3 o = new Vec3(current.origin.x, current.origin.y - 0.5f, current.origin.z);
			obj3.Add(new MatrixFrame(in rotation4, in o));
		}
	}
	...
	public void GetBanditFrames(out List<MatrixFrame> initialFrames, out List<MatrixFrame> targetFrames, MatrixFrame initialBossFrame, MatrixFrame targetBossFrame, int agentCount, float agentOffsetAngle)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			Vec3 o = new Vec3(initialBossFrame.origin.x, initialBossFrame.origin.y + 1.2f * (float)num, initialBossFrame.origin.z);
			array[num2] = new MatrixFrame(in rotation, in o);
		}
		for (int j = 0; j < array.Length; j++)
		{
