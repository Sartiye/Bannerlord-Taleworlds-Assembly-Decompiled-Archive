...
public class SandboxAutoBlockModel : AutoBlockModel
{
	public override Agent.UsageDirection GetBlockDirection(Mission mission)
	{
		...
		foreach (Agent agent in mission.Agents)
		{
			...
			if ((currentActionStage != 0 && currentActionStage != Agent.ActionStage.AttackQuickReady && currentActionStage != Agent.ActionStage.AttackRelease) || !agent.IsEnemyOf(mainAgent))
			{
				continue;
			}
