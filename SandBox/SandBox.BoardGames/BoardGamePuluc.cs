...
public class BoardGamePuluc : BoardGameBase
{
	...
	public override void InitializeTiles()
	{
		...
		while (x2 < 11)
		{
			GameEntity gameEntity = source.Single((GameEntity e) => e.HasTag("tile_" + x2));
			BoardGameDecal firstScriptOfType = source2.Single((GameEntity e) => e.HasTag("decal_" + x2)).GetFirstScriptOfType<BoardGameDecal>();
			base.Tiles[x2] = new TilePuluc(gameEntity, firstScriptOfType, x2);
		...
		firstChildEntityWithTag2.CreateVariableRatePhysics(forChildren: true);
	}
	...
	protected override void MovePawnToTile(PawnBase pawn, TileBase tile, bool instantMove = false, bool displayMessage = true)
	{
		...
		Vec3 goal3 = ((pawnPuluc.State != 0) ? (pawn.PlayerOne ? tilePuluc.PosRightMid : tilePuluc.PosLeftMid) : (pawn.PlayerOne ? tilePuluc.PosRight : tilePuluc.PosLeft));
