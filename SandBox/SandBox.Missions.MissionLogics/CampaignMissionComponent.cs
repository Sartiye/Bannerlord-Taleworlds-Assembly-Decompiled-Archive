...
public class CampaignMissionComponent : MissionLogic, ICampaignMission
{
	...
	private void SetMountAgentAnimation(IAgent agent, ConversationAnimData mountAnimData, bool startingConversation)
	{
		...
		if (mountAnimData != null)
		{
			...
			SetConversationAgentActionAtChannel(agent2, in action, GetActionChannelNoForConversation(agent2), setInstantly: false, forceFaceMorphRestart: false);
		}
	}
	...
	private void StartConversationAnimations(IAgent iAgent, bool setActionsInstantly)
	{
		...
		if (Campaign.Current.ConversationManager.ConversationAnimationManager.ConversationAnims.TryGetValue(idleAnimationId, out var value))
		{
			...
			SetConversationAgentActionAtChannel(agent, in action, actionChannelNoForConversation, setActionsInstantly, forceFaceMorphRestart: false);
	...
	void ICampaignMission.OnConversationPlay(string idleActionId, string idleFaceAnimId, string reactionId, string reactionFaceAnimId, string soundPath)
	{
		...
		if (!string.IsNullOrEmpty(idleAnimationId) && Campaign.Current.ConversationManager.ConversationAnimationManager.ConversationAnims.TryGetValue(idleAnimationId, out var value))
		{
			...
			{
				Agent currentAgent = _currentAgent;
				ActionIndexCache action = ActionIndexCache.Create(value.Reactions[reactionId]);
				SetConversationAgentActionAtChannel(currentAgent, in action, 0, setInstantly: false, forceFaceMorphRestart: true);
			}
			else
			{
				ActionIndexCache action2 = (string.IsNullOrEmpty(value.IdleAnimStart) ? ActionIndexCache.Create(value.IdleAnimLoop) : ActionIndexCache.Create(value.IdleAnimStart));
				SetConversationAgentActionAtChannel(_currentAgent, in action2, GetActionChannelNoForConversation(_currentAgent), setInstantly: false, forceFaceMorphRestart: false);
			}
		}
