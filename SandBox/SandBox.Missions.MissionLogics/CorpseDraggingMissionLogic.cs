...
public class CorpseDraggingMissionLogic : MissionLogic, IPlayerInputEffector, IMissionBehavior
{
	...
	{
		Agent mainAgent = base.Mission.MainAgent;
		if (_draggedCorpse == null || _crouchStandEvent != 0 || (mainAgent.GetCurrentActionType(1) == Agent.ActionCodeType.EquipUnequip && _startedPickingUpDraggedCorpse) || _draggedCorpse.AgentVisuals.GetCurrentRagdollState() != RagdollState.Active)
		{
			return;
		}
		...
		if (_triggerBodyGrabSound)
		{
			...
			Vec3 position = _draggedCorpse.Position;
			SoundManager.StartOneShotEvent("event:/physics/bodydrag/human/grab/body_grab", in position, "Armor Type", soundParameterForArmorType);
