...
public class CheckpointMissionLogic : MissionLogic
{
	...
	public override void OnEarlyAgentRemoved(Agent affectedAgent, Agent affectorAgent, AgentState agentState, KillingBlow blow)
	{
		...
		foreach (KeyValuePair<Agent, AgentSaveData> allSpawnedSaveableAgent in _allSpawnedSaveableAgents)
		{
			if (allSpawnedSaveableAgent.Key == affectedAgent)
			{
				...
				Mat3 rot = allSpawnedSaveableAgent.Key.LookRotation;
				Vec3 o = allSpawnedSaveableAgent.Key.Position;
				value.UpdateSpawnFrame(new MatrixFrame(in rot, in o));
				break;
			}
		}
	}
	...
	private void SpawnCorpses()
	{
		foreach (AgentSaveData corpse in _checkpointCampaignBehavior.CorpseList)
		{
			...
			CharacterObject @object = MBObjectManager.Instance.GetObject<CharacterObject>(current.CharacterStringId);
			AgentBuildData agentBuildData = new AgentBuildData(@object).TroopOrigin(new SimpleAgentOrigin(@object)).EquipmentSeed(current.AgentSeed).InitialPosition(in current.SpawnFrame.origin);
			Vec2 direction = current.SpawnFrame.rotation.f.NormalizedCopy().AsVec2;
			AgentBuildData agentBuildData2 = agentBuildData.InitialDirection(in direction);
	...
	public void RegisterAgent(Agent agent)
	{
		...
		Mat3 rot = agent.LookRotation;
		Vec3 o = agent.Position;
		allSpawnedSaveableAgents.Add(agent, new AgentSaveData(stringId, new MatrixFrame(in rot, in o), agent.AgentVisuals.GetEntity().Tags, agent.Origin.Seed));
	}
}
