...
public class MapConversationTableau
{
	...
	private void SpawnOpponentLeader()
	{
		...
		ActionIndexCache actionCode = ActionIndexCache.Create(defaultAnimForCharacter.ActionName);
		AgentVisuals agentVisuals = AgentVisuals.Create(agentVisualsData.ActionCode(in actionCode).Scene(_tableauScene).Monster(baseMonsterFromRace)
	...
	private void SpawnOpponentBodyguardCharacter(CharacterObject character, int indexOfBodyguard, PartyBase party)
	{
		if (indexOfBodyguard >= 0 && indexOfBodyguard <= 1)
		{
			...
			Monster baseMonsterFromRace = TaleWorlds.Core.FaceGen.GetBaseMonsterFromRace(character.Race);
			AgentVisualsData agentVisualsData = new AgentVisualsData().Banner(_data.ConversationPartnerData.Party?.LeaderHero?.ClanBanner).Equipment(equipment).Race(character.Race)
	...
	public void OnConversationPlay(string idleActionId, string idleFaceAnimId, string reactionId, string reactionFaceAnimId, string soundPath)
	{
		...
		if (!Campaign.Current.ConversationManager.SpeakerAgent.Character.IsPlayerCharacter)
		{
			...
			if (flag)
			{
				...
				{
					AgentVisuals agentVisuals = _agentVisuals[0];
					ActionIndexCache actionIndex = ActionIndexCache.Create(value.Reactions[reactionId]);
					agentVisuals.SetAction(in actionIndex, 0f, forceFaceMorphRestart: false);
				}
				else if (!flag2 || _changeIdleActionTimer.Check(Game.Current.ApplicationTime))
				{
					ActionIndexCache actionIndex2 = ActionIndexCache.Create(value.IdleAnimStart);
					if (!_agentVisuals[0].DoesActionContinueWithCurrentAction(in actionIndex2))
					{
						_changeIdleActionTimer.Reset(Game.Current.ApplicationTime);
						_agentVisuals[0].SetAction(in actionIndex2, 0f, forceFaceMorphRestart: false);
					}
				}
			}
