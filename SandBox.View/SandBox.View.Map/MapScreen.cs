...
public class MapScreen : ScreenBase, IMapStateHandler, IGameStateListener, IChatLogHandlerScreen
{
	...
	private MouseInputState GetMouseInputState()
	{
		...
		MouseInputState result = default(MouseInputState);
		result.IsLeftMousePressed = SceneLayer.Input.IsKeyPressed(InputKey.LeftMouseButton);
		result.IsLeftMouseDown = SceneLayer.Input.IsKeyDown(InputKey.LeftMouseButton);
		result.IsLeftMouseReleased = SceneLayer.Input.IsKeyReleased(InputKey.LeftMouseButton);
		result.IsMiddleMousePressed = SceneLayer.Input.IsKeyPressed(InputKey.MiddleMouseButton);
		result.IsMiddleMouseDown = SceneLayer.Input.IsKeyDown(InputKey.MiddleMouseButton);
		result.IsMiddleMouseReleased = SceneLayer.Input.IsKeyReleased(InputKey.MiddleMouseButton);
		result.IsRightMousePressed = SceneLayer.Input.IsKeyPressed(InputKey.RightMouseButton);
		result.IsRightMouseDown = SceneLayer.Input.IsKeyDown(InputKey.RightMouseButton);
		result.IsRightMouseReleased = SceneLayer.Input.IsKeyReleased(InputKey.RightMouseButton);
		return result;
	}
	...
	private void TickCircles()
	{
		...
		if (MobileParty.MainParty.PartyMoveMode == MoveModeType.Point && MobileParty.MainParty.DefaultBehavior != AiBehavior.GoToSettlement && MobileParty.MainParty.DefaultBehavior != 0 && !MobileParty.MainParty.ForceAiNoPathMode && MobileParty.MainParty.Ai.AiBehaviorInteractable == null && MobileParty.MainParty.MapEvent == null && MobileParty.MainParty.TargetPosition.DistanceSquared(MobileParty.MainParty.Position) > 0.01f)
		{
			flag3 = true;
			flag = true;
			num = 0.238846f;
		...
		Vec3 scaleAmountXYZ = new Vec3(num, num, num);
