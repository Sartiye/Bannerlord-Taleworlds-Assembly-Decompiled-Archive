...
public class MapCameraView : MapView
{
	...
	protected virtual void UpdateMapCamera(bool _leftButtonDraggingMode, Vec3 _clickedPosition)
	{
		...
		float height = 0f;
		IMapScene mapSceneWrapper = Campaign.Current.MapSceneWrapper;
		CampaignVec2 point = new CampaignVec2(_cameraFrame.origin.AsVec2, !isCurrentlyAtSea);
		mapSceneWrapper.GetHeightAtPoint(in point, ref height);
		height += 0.5f;
		if (_cameraFrame.origin.z < height)
		{
			...
			else
			{
				...
				do
				{
					...
					point = new CampaignVec2(_cameraFrame.origin.AsVec2, !isCurrentlyAtSea);
					mapSceneWrapper2.GetHeightAtPoint(in point, ref height);
					height += 0.5f;
					if (num > 0.0001f)
					{
						num *= 0.5f;
				...
				if (_cameraFrame.origin.z < height)
				{
					...
					if (MBMath.GetRayPlaneIntersectionPoint(in planeNormal2, in planeCenter2, in _cameraFrame.origin, in rayDirection2, out var t2) && CurrentCameraFollowMode != CameraFollowMode.MoveToPosition)
					{
		...
		point = new CampaignVec2(identity.origin.AsVec2, isOnLand: true);
		mapSceneWrapper3.GetHeightAtPoint(in point, ref identity.origin.z);
	...
	protected virtual bool GetMapCameraInput(InputInformation inputInformation)
	{
		...
		if (CurrentCameraFollowMode == CameraFollowMode.Free && !inputInformation.RightMouseButtonDown && !inputInformation.LeftMouseButtonDown && num3 >= 0.001f && (inputInformation.WorldMouseFar - CameraFrame.origin).NormalizedCopy().z < -0.2f && inputInformation.RayCastForClosestEntityOrTerrainCondition)
		{
			...
			ref Mat3 rotation = ref matrixFrame.rotation;
			Vec3 v2 = CameraFrame.rotation.TransformToLocal(in v);
			Vec3 rayDirection2 = rotation.TransformToParent(in v2);
