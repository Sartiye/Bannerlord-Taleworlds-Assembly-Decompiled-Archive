...
public class SettlementVisual : MapEntityVisual<PartyBase>
{
	...
	internal void Tick(float dt, ref int dirtyPartiesCount, ref SettlementVisual[] dirtyPartiesList)
	{
		...
		else
		{
				...
				if (siegeEventSide.SiegeEngineMissiles.Count > item2)
				{
					...
					if (toHours > toHours2 - (double)bombardmentData.TotalDuration)
					{
						...
						if (flag3)
						{
							...
							rot.f = vec3 - o;
							rot.Orthonormalize();
							Vec3 scaleAmountXYZ = base.MapScreen.PrefabEntityCache.GetScaleForSiegeEngine(siegeEngineMissile.ShooterSiegeEngineType, siegeEventSide.BattleSide);
							rot.ApplyScaleLocal(in scaleAmountXYZ);
									...
									foreach (var siegeRangedMachineEntity in _siegeRangedMachineEntities)
									{
										if (!flag && siegeRangedMachineEntity.Item2 == siegeEventSide.BattleSide && siegeRangedMachineEntity.Item3 == siegeEngineMissile.ShooterSlotIndex)
										{
											...
											if (item3 != null)
											{
												...
												MatrixFrame m = MBSkeletonExtensions.GetBoneEntitialFrame(item3.Skeleton, Campaign.Current.Models.SiegeEventModel.GetSiegeEngineMapProjectileBoneIndex(siegeEngineMissile.ShooterSiegeEngineType, siegeEventSide.BattleSide));
												globalFrame = globalFrame.TransformToParent(in m);
												item.SetGlobalFrame(in globalFrame);
											}
										}
									}
								}
							...
							else if (toHours < toHours2 - (double)(bombardmentData.TotalDuration - bombardmentData.RotationDuration - bombardmentData.ReloadDuration - bombardmentData.AimingDuration - bombardmentData.FireDuration) && !flag3 && siegeEventSide.SiegeEngines.DeployedRangedSiegeEngines[siegeEngineMissile.ShooterSlotIndex] != null && siegeEventSide.SiegeEngines.DeployedRangedSiegeEngines[siegeEngineMissile.ShooterSlotIndex].SiegeEngine == siegeEngineMissile.ShooterSiegeEngineType)
							{
								foreach (var siegeRangedMachineEntity2 in _siegeRangedMachineEntities)
								{
									if (!flag && siegeRangedMachineEntity2.Item2 == siegeEventSide.BattleSide && siegeRangedMachineEntity2.Item3 == siegeEngineMissile.ShooterSlotIndex)
									{
										...
										{
											flag = true;
											MatrixFrame m = item4.GetGlobalFrame();
											MatrixFrame globalFrame = MBSkeletonExtensions.GetBoneEntitialFrame(item4.Skeleton, Campaign.Current.Models.SiegeEventModel.GetSiegeEngineMapProjectileBoneIndex(siegeEngineMissile.ShooterSiegeEngineType, siegeEventSide.BattleSide));
											m = m.TransformToParent(in globalFrame);
											item.SetGlobalFrame(in m);
										}
									}
								}
							}
			...
			foreach (var siegeRangedMachineEntity3 in _siegeRangedMachineEntities)
			{
				...
				MatrixFrame frame2 = bombardmentData2.ShooterGlobalFrame;
				if (rangedSiegeEngine.PreviousTargetIndex >= 0)
				{
					...
					frame2.rotation.f.AsVec2 = (vec4 - frame2.origin).AsVec2;
					frame2.rotation.f.NormalizeWithoutChangingZ();
					frame2.rotation.Orthonormalize();
				}
				item5.SetGlobalFrame(in frame2);
				...
				{
					float rotationInRadians = (bombardmentData2.TargetPosition - frame2.origin).AsVec2.RotationInRadians;
					float rotationInRadians2 = frame2.rotation.f.AsVec2.RotationInRadians;
					...
					if (num6 > num7 * 2f)
					{
						frame2.rotation.f.AsVec2 = Vec2.FromRotation(rotationInRadians2 + (float)TaleWorlds.Library.MathF.Sign(f) * (num6 - num7 * 2f));
						frame2.rotation.f.NormalizeWithoutChangingZ();
						frame2.rotation.Orthonormalize();
						item5.SetGlobalFrame(in frame2);
					}
	...
	private void CalculateDataAndDurationsForSiegeMachine(int machineSlotIndex, SiegeEngineType machineType, BattleSideEnum side, SiegeBombardTargets targetType, int targetSlotIndex, out SiegeBombardmentData bombardmentData)
	{
		...
		default:
		{
			...
			IMapScene mapSceneWrapper = Campaign.Current.MapSceneWrapper;
			CampaignVec2 point = new CampaignVec2(bombardmentData.TargetPosition.AsVec2, isOnLand: true);
			mapSceneWrapper.GetHeightAtPoint(in point, ref bombardmentData.TargetPosition.z);
			break;
		}
		}
