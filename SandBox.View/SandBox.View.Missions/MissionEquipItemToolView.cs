...
public class MissionEquipItemToolView : MissionView
{
	...
	public override void OnMissionTick(float dt)
	{
				...
				if (base.DebugInput.IsKeyPressed(InputKey.F))
				{
					if (_currentItems.Count > 0)
					{
						foreach (ItemObject allItemObject in _allItemObjects)
						{
							...
							AgentBuildData agentBuildData = new AgentBuildData(new SimpleAgentOrigin(character)).Equipment(equipment).NoHorses(noHorses: true).Team(base.Mission.DefenderTeam);
				...
				else if (base.DebugInput.IsKeyPressed(InputKey.BackSpace))
				{
					...
					else
					{
						...
						default:
						{
							...
							AgentBuildData agentBuildData3 = new AgentBuildData(new SimpleAgentOrigin(character2)).Equipment(spawnEquipment).NoHorses(noHorses: true).Team(base.Mission.DefenderTeam);
							Vec3 position = new Vec3(500f, 200f, 1f);
		...
		if (base.DebugInput.IsKeyDown(InputKey.LeftAlt) && !base.DebugInput.IsKeyDown(InputKey.LeftControl) && (base.DebugInput.IsKeyPressed(InputKey.MouseScrollUp) || base.DebugInput.IsKeyPressed(InputKey.MouseScrollDown)))
		{
			...
			Vec2 direction = frame6.origin.AsVec2;
			mainAgent.SetTargetPositionAndDirection(in direction, in frame6.rotation.f);
		}
	}
	...
	private void SpawnItems()
	{
		...
		foreach (ItemObject allItemObject in _allItemObjects)
		{
			...
			Vec3 o = _mainAgent.Position + new Vec3(-1f, 1f) + Vec3.Up * num;
			MatrixFrame frame2 = new MatrixFrame(in rotation, in o);
	...
	private void SpawnAgent(string id)
	{
		...
		list.AddRange(@object.CivilianEquipments.ToList());
		list.AddRange(@object.StealthEquipments.ToList());
		Mission mission = base.Mission;
		AgentBuildData agentBuildData = new AgentBuildData(new SimpleAgentOrigin(@object)).Equipment(list[_setIndex]).NoHorses(noHorses: true).Team(base.Mission.DefenderTeam);
		Vec3 position = new Vec3(500f, 200f, 1f);
	...
	private void PositionCurrentItems()
	{
		...
		foreach (ItemData currentItem in _currentItems)
		{
			...
			Vec3 o = _mainAgent.Position + new Vec3(-1f, 1f + num, z) + Vec3.Up * num2;
