...
public static class TroopFilteringUtilities
{
	...
	{
		int num = 1;
		if ((filter & TroopTraitsMask.HighTier) != 0)
		{
			num += battleTier;
		}
		if ((filter & TroopTraitsMask.LowTier) != 0)
		{
			num += 7 - battleTier;
		}
		TroopTraitsMask troopTraitsMask = filter & troopMask;
		if ((troopTraitsMask & TroopTraitsMask.Shield) != 0)
		{
			num += 10;
		}
		if ((troopTraitsMask & TroopTraitsMask.Spear) != 0)
		{
			num += 10;
		}
		if ((troopTraitsMask & TroopTraitsMask.Thrown) != 0)
		{
			num += 10;
		}
		if ((troopTraitsMask & TroopTraitsMask.Armor) != 0)
		{
			num += 10;
		}
		if ((troopTraitsMask & TroopTraitsMask.Melee) != 0 || (troopTraitsMask & TroopTraitsMask.Ranged) != 0)
		{
			num += 100;
		}
		if ((troopTraitsMask & TroopTraitsMask.Mount) != 0)
		{
			num += 1000;
		}
		return num;
	...
	{
		return 1 + (((filter & TroopTraitsMask.HighTier) != 0) ? 7 : 0) + (((filter & TroopTraitsMask.LowTier) != 0) ? 7 : 0) + (((filter & TroopTraitsMask.Shield) != 0) ? 10 : 0) + (((filter & TroopTraitsMask.Spear) != 0) ? 10 : 0) + (((filter & TroopTraitsMask.Thrown) != 0) ? 10 : 0) + (((filter & TroopTraitsMask.Armor) != 0) ? 10 : 0) + (((filter & TroopTraitsMask.Melee) != 0 || (filter & TroopTraitsMask.Ranged) != 0) ? 100 : 0) + (((filter & TroopTraitsMask.Mount) != 0) ? 1000 : 0);
	}
}
