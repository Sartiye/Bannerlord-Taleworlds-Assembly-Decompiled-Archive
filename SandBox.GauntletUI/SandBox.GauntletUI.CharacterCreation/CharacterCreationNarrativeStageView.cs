...
public class CharacterCreationNarrativeStageView : CharacterCreationStageViewBase
{
	...
	private void SpawnNonHumanNarrativeMenuCharacter(NarrativeMenuCharacter character)
	{
		...
		MountVisualCreator.AddMountMeshToEntity(gameEntity2, @object, object2, character.MountCreationKey.ToString());
		...
		if (character.IsHuman)
		{
			if (!string.IsNullOrEmpty(character.Item1Id))
			{
				...
				if (@object != null)
				{
					...
					equipment.AddEquipmentToSlotWithoutAgent(equipmentIndex, new EquipmentElement(@object));
				}
			}
			if (!string.IsNullOrEmpty(character.Item2Id))
			{
				...
				if (object2 != null)
				{
					...
					equipment.AddEquipmentToSlotWithoutAgent(equipmentIndex2, new EquipmentElement(object2));
				}
			}
		}
