...
public class NavalCustomBattleTroopTypeSelectionPopUpVM : ViewModel
{
	...
	public void OpenPopUp(string title, MBBindingList<NavalCustomBattleTroopTypeVM> troops)
	{
		...
		foreach (NavalCustomBattleTroopTypeVM troop in troops)
		{
			_itemSelectionsBackUp.Add(troop.IsSelected);
		}
		_selectedItemCount = troops.Count((NavalCustomBattleTroopTypeVM x) => x.IsSelected);
		Title = title;
		Items = troops;
		IsOpen = true;
	}
	...
	{
		Items.ApplyActionOnAllItems(delegate(NavalCustomBattleTroopTypeVM x)
		{
			x.IsSelected = true;
		});
	...
	public void ExecuteBackToDefault()
	{
		...
		_selectedItemCount = Items.Count((NavalCustomBattleTroopTypeVM x) => x.IsSelected);
	}
