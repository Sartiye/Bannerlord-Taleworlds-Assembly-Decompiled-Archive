	...
	public override void Deserialize(MBObjectManager objectManager, XmlNode node)
	{
		...
		foreach (XmlNode childNode in node.ChildNodes)
		{
			...
			foreach (XmlNode childNode2 in childNode.ChildNodes)
			{
				...
				{
					Debug.FailedAssert("ShipSlot does not exist! Ship slot id: " + value + "\nShip slot cannot be added!", "C:\\BuildAgent\\work\\mb3\\Source\\Bannerlord\\TaleWorlds.Core\\NavalCore\\ShipHull.cs", "Deserialize", 149);
				}
			}
		}
		...
		float num = DeserializeScalarAttribute<float>(node, "floating_force_multiplier", isRequiredAttribute: false, out isAttributeValid);
		FloatingForceMultiplier = (isAttributeValid ? num : 0.451f);
	}
