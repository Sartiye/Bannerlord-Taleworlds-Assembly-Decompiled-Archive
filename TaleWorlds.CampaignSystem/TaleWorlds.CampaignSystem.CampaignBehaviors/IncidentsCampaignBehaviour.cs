...
public class IncidentsCampaignBehaviour : CampaignBehaviorBase, INonReadyObjectHandler
{
	...
	private void ConversationEnded(IEnumerable<CharacterObject> conversationCharacters)
	{
		...
		{
			TryInvokeIncident(new Incident.IncidentTrigger("LeavingEncounter"));
		}
	}

	private void OnMapEventEnded(MapEvent evt)
	{
		if (evt.IsPlayerMapEvent && !evt.IsNavalMapEvent && evt.HasWinner && evt.DefeatedSide != evt.PlayerSide && (evt.IsFieldBattle || evt.IsHideoutBattle) && !evt.AttackerSide.IsSurrendered && !evt.DefenderSide.IsSurrendered && MobileParty.MainParty.RandomFloatWithSeed((uint)CampaignTime.Now.ToMinutes) < Campaign.Current.Models.IncidentModel.GetIncidentTriggerGlobalProbability())
		{
			TryInvokeIncident(new Incident.IncidentTrigger("LeavingBattle"));
		}
	}

	private void OnGameMenuOpened(MenuCallbackArgs args)
	{
		if (_canInvokeSettlementEvent && MobileParty.MainParty.RandomFloatWithSeed((uint)CampaignTime.Now.ToMinutes) < Campaign.Current.Models.IncidentModel.GetIncidentTriggerGlobalProbability())
		{
			if (args.MenuContext.GameMenu.StringId == "town")
			{
				TryInvokeIncident(new Incident.IncidentTrigger("EnteringTown"));
				_canInvokeSettlementEvent = false;
			}
			if (args.MenuContext.GameMenu.StringId == "village")
			{
				TryInvokeIncident(new Incident.IncidentTrigger("EnteringVillage"));
				_canInvokeSettlementEvent = false;
			}
			if (args.MenuContext.GameMenu.StringId == "castle")
			{
				TryInvokeIncident(new Incident.IncidentTrigger("EnteringCastle"));
				_canInvokeSettlementEvent = false;
			}
		}
	}
	...
	private void OnGameMenuOptionSelected(GameMenu gameMenu, GameMenuOption option)
	{
		if (MobileParty.MainParty.RandomFloatWithSeed((uint)CampaignTime.Now.ToMinutes) < Campaign.Current.Models.IncidentModel.GetIncidentTriggerGlobalProbability())
		{
			...
			{
				TryInvokeIncident(new Incident.IncidentTrigger("LeavingTown", "LeavingTown"));
			}
			if (gameMenu.StringId == "castle" && option.IdString == "leave")
			{
				TryInvokeIncident(new Incident.IncidentTrigger("LeavingCastle", "LeavingCastle"));
			}
			if (gameMenu.StringId == "village" && option.IdString == "leave")
			{
				TryInvokeIncident(new Incident.IncidentTrigger("LeavingVillage", "LeavingVillage"));
			}
		}
	}
	...
	private void OnHourlyTick()
	{
		...
		{
			TryInvokeIncident(new Incident.IncidentTrigger("DuringSiege"));
		}
		...
		{
			TryInvokeIncident(new Incident.IncidentTrigger("WaitingInSettlement"));
		}
	}

	private void TryInvokeIncident(Incident.IncidentTrigger trigger)
	{
		if (((!trigger.HasTrigger("EnteringCastle") && !trigger.HasTrigger("EnteringTown") && !trigger.HasTrigger("EnteringVillage")) || (MobileParty.MainParty.CurrentSettlement != null && !MobileParty.MainParty.CurrentSettlement.IsSettlementBusy(this))) && ((!trigger.HasTrigger("LeavingTown") && !trigger.HasTrigger("LeavingVillage") && !trigger.HasTrigger("LeavingCastle")) || !MobileParty.MainParty.LastVisitedSettlement.IsSettlementBusy(this)) && !Campaign.Current.ConversationManager.IsConversationFlowActive)
		{
	...
	private void InitializeIncidents()
	{
		...
		incident.AddOption("{=Ppzvp7Mu}Tell the peasant that the rooster is just part of the debt they owe their protectors.", new List<IncidentEffect> { CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, -10) });
		incident.AddOption("{=NdcsYWrF}Throw the peasant a few silvers in compensation.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.GoldChange(() => -10),
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, 100),
			CampaignIncidentEffects.Group(CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => true, () => 1), CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, 10)).WithChance(0.5f)
		});
		incident2.AddOption("{=bjDjwN9r}Congratulate the lad on finding a wife and throw the couple a purse of coins as a wedding gift", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => true, () => 1)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.ChangeItemAmount(GetSpoiledFoodItem, () => -2)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 100),
			CampaignIncidentEffects.ChangeItemAmount(GetDesperateTimesFoodItem, () => -1)
		});
		...
		incident4.AddOption("{=BslqBNQj}Tell them that you cannot help", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100) });
		...
		incident5.AddOption("{=yfsqkUcb}Tell the townspeople that Heaven wishes to test their compassion, and that they should treat the lepers with kindness.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 200),
			CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, -5),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, -10)
		});
		...
		incident6.AddOption("{=56hS9BRG}Admonish the {HIGH_TIER_TROOPS} for their indiscipline, even though they will be humiliated and may even desert", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, 100),
			CampaignIncidentEffects.MoraleChange(5f),
			CampaignIncidentEffects.ChangeTroopAmount(GetVeteranMaxTierTroop, -1).WithChance(0.5f)
		}, delegate(TextObject text)
		{
			CharacterObject veteranMaxTierTroop = GetVeteranMaxTierTroop();
			text.SetTextVariable("HIGH_TIER_TROOPS", veteranMaxTierTroop.Name);
			return true;
		});
		...
		incident6.AddOption("{=37YTPzfG}Tell your men that they have all earned the right to your esteem, and break out an extra ration of {FOOD_OR_DRINK_ITEM} to show your appreciation", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.MoraleChange(5f),
			CampaignIncidentEffects.ChangeItemAmount(GetVeteranFoodItem, () => -1)
		}, delegate(TextObject text)
		{
			ItemObject veteranFoodItem = GetVeteranFoodItem();
			text.SetTextVariable("FOOD_OR_DRINK_ITEM", veteranFoodItem.Name);
			return true;
		});
		...
		Incident incident7 = RegisterIncident("incident_an_affair_of_honor", "{=gUKUisS6}An Affair of Honor", "{=akvIjedh}Two of your cavalrymen request permission to fight a duel. Both had been wooing a merchant's daughter in {TOWN}, and one slighted the other in front of her. Some commanders encourage their men to duel as a boost to their fiery sense of pride, but others think that such fights erode their discipline as soldiers and, of course, carry the risk of injury.", "LeavingVillage", "PartyCampLife", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			if (MobileParty.MainParty.LastVisitedSettlement == null)
			{
				return false;
			}
			bool num14 = (from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
				where x.Character.Tier >= 4 && x.Character.HasMount() && !x.Character.IsHero
				select x).Sum((TroopRosterElement x) => x.Number) >= 5;
			bool flag5 = MobileParty.MainParty.MemberRoster.GetTroopRoster().Count((TroopRosterElement x) => x.Character.Tier >= 4 && x.Character.HasMount() && !x.Character.IsHero) >= 2;
			if (!num14 || !flag5)
			{
				return false;
			}
		...
		incident7.AddOption("{=24pUil64}You like that fiery spirit! Let them fight until one falls and cannot rise", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.WoundTroop(GetFirstNobleTroop, 1).WithChance(0.5f),
			CampaignIncidentEffects.KillTroop(GetSecondNobleTroop, 1).WithChance(0.5f)
		});
		incident7.AddOption("{=37lEhqtv}Let them fight until the first drop of blood is shed", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.WoundTroop(GetFirstNobleTroop, 1).WithChance(0.5f),
			CampaignIncidentEffects.WoundTroop(GetSecondNobleTroop, 1).WithChance(0.5f)
		});
		...
		incident7.AddOption("{=ZYopfLDg}Tell your men to throw a die, leaving the matter to fate.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100) });
		...
		{
			CharacterObject insultCavalryman = GetInsultCavalryman();
			CharacterObject insultFootman = GetInsultFootman();
			if (insultFootman == null || insultCavalryman == null)
			{
				return false;
			}
			description.SetTextVariable("CAVALRYMAN", insultCavalryman.Name);
			description.SetTextVariable("FOOTMAN", insultFootman.Name);
		...
		incident8.AddOption("{=QGcozvvK}Only blood can wipe out an insult! Let the two have it out, until one falls and cannot rise", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 200),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100),
			CampaignIncidentEffects.Select((CampaignIncidentEffects.KillTroopsRandomly(FootmenTroopsPredicate, () => 1), 0.25f), (CampaignIncidentEffects.WoundTroop(GetInsultCavalryman, 1), 0.25f), (CampaignIncidentEffects.KillTroopsRandomly(CavalryTroopsPredicate, () => 1), 0.25f), (CampaignIncidentEffects.WoundTroop(GetInsultFootman, 1), 0.25f)),
			CampaignIncidentEffects.PartyExperienceChance(200)
		});
		incident8.AddOption("{=HbQDtv60}Reprimand the cavalryman, even though he is fiery and might desert", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 100),
			CampaignIncidentEffects.MoraleChange(5f),
			CampaignIncidentEffects.KillTroopsRandomly(CavalryTroopsPredicate, () => 1).WithChance(0.25f)
		});
		...
		incident9.AddOption("{=CXZxDvry}Tell your soldiers they should rely on their own strength instead of magic", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 100) });
		...
		{
			CampaignIncidentEffects.ChangeItemAmount(GetPurebredHorseItem, () => -1),
			CampaignIncidentEffects.ChangeItemAmount(() => Game.Current.ObjectManager.GetObject<ItemObject>("mule"), () => 1),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100)
		});
		...
		{
			CampaignIncidentEffects.MoraleChange(5f),
			CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => x.Character.HasMount() && !x.Character.IsHero && !x.Character.IsHero, () => 1).WithChance(0.25f)
		});
		...
		{
			CampaignIncidentEffects.GoldChange(() => -500),
			CampaignIncidentEffects.PartyExperienceChance(400),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100),
			CampaignIncidentEffects.WoundTroopsRandomly(1)
		});
		...
		incident12.AddOption("{=SIVBGe7H}Isolate the infected horses immediately to prevent the spread ", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.ChangeItemsAmount(GetHorseflyPlagueHorseItems, -1),
			CampaignIncidentEffects.ChangeItemsAmount(GetHorseflyPlagueHorseItems, -3).WithChance(0.3f),
			CampaignIncidentEffects.DisorganizeParty()
		});
		...
		{
			CampaignIncidentEffects.DemoteTroopsRandomlyWithPredicate(TroopsToDemotePredicate, TroopToDemoteToPredicate, 1),
			CampaignIncidentEffects.Group(CampaignIncidentEffects.DemoteTroopsRandomlyWithPredicate(TroopsToDemotePredicate, TroopToDemoteToPredicate, 3), CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => true, () => 2)).WithChance(0.25f)
		});
		incident13.AddOption("{=bWz49lZu}Allow the cavalryman to care for the animal, even though it puts him and others at risk.", new List<IncidentEffect> { CampaignIncidentEffects.Select(CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 100), CampaignIncidentEffects.Group(CampaignIncidentEffects.DemoteTroopsRandomlyWithPredicate(TroopsToDemotePredicate, TroopToDemoteToPredicate, 3), CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => true, () => 2), CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -200)).WithChance(0.25f), 0.5f) });
		Incident incident14 = RegisterIncident("incident_colicky_horses", "{=k4kbVxTL}Colicky Horses", "{=Bi8fKaGw}As you leave town, one of your cavalrymen finds his mount lying on the ground, whining in pain and refusing to get up. You're sure that he was careless and fed the animal too quickly after your arrival, as he was eager to get to the town's tavern.", "LeavingTown", "AnimalIllness", CampaignTime.Years(1000f), (TextObject description) => MobileParty.MainParty.MemberRoster.GetTroopRoster().Where(TroopToDemotePredicate).Sum((TroopRosterElement x) => x.Number) >= 15);
		...
		{
			CampaignIncidentEffects.DisorganizeParty(),
			CampaignIncidentEffects.DemoteTroopsRandomlyWithPredicate(TroopToDemotePredicate, TroopToDemoteToPredicate, 1).WithChance(0.5f)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 100),
			CampaignIncidentEffects.DemoteTroopsRandomlyWithPredicate(TroopToDemotePredicate, TroopToDemoteToPredicate, 1)
		});
		...
		{
			CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => true, () => 1).WithChance(0.1f),
			CampaignIncidentEffects.WoundTroopsRandomly(2).WithChance(0.6f)
		});
		...
		incident18.AddOption("{=Ioc2z1Ap}Declare that you owe your victory to no saint but to your men, and give them the silver", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 200),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100),
			CampaignIncidentEffects.MoraleChange(3f),
			CampaignIncidentEffects.GoldChange(() => -300)
		});
		...
		Incident incident20 = RegisterIncident("incident_right_of_pillage", "{=p0bMRIum}Right of Pillage", "{=NWJ87xjB}As you prepare to march forth from newly captured {TOWN_NAME}, your men stop you. They shed their blood for your wars and you denied them their customary right to sack it. They say that it you will not let them enrich themselves from the enemy, then you should enrich them from your purse", new string[2] { "LeavingTown", "LeavingCastle" }, "PartyCampLife", CampaignTime.Days(60f), delegate(TextObject description)
		{
			if (MobileParty.MainParty.LastVisitedSettlement == null)
			{
				return false;
			}
			BesiegeSettlementLogEntry besiegeSettlementLogEntry2 = FindLastBesiegeSettlementLogEntry();
			if (besiegeSettlementLogEntry2 == null)
			{
				return false;
			}
			SiegeAftermathLogEntry siegeAftermathLogEntry = FindLastSiegeAftermathLogEntry(besiegeSettlementLogEntry2);
			if (siegeAftermathLogEntry == null)
			{
				return false;
			}
			if (siegeAftermathLogEntry.GameTime < besiegeSettlementLogEntry2.GameTime)
			{
				return false;
			}
		...
		incident20.AddOption("{=lm37mXGH}Let your men return and sack the town while it is still defenseless", new List<IncidentEffect> { CampaignIncidentEffects.Custom(null, delegate
		{
			...
			return new List<TextObject>();
		}, (IncidentEffect effect) => new IncidentHint(new TextObject("{=6QuPiTsY}Consequences of a town sacked"))) });
		...
		incident20.AddOption("{=HG7WWmX8}Punish the troublemakers for sewing mutiny", new List<IncidentEffect> { CampaignIncidentEffects.MoraleChange(-20f) });
		...
		incident23.AddOption("{=p8yBnCG2}Let your men enjoy their meal, but commend the villagers for making a good attempt to get their hands on a piece of perfectly good meat", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 100),
			CampaignIncidentEffects.MoraleChange(5f),
			CampaignIncidentEffects.ChangeItemAmount(() => Game.Current.ObjectManager.GetObject<ItemObject>("meat"), () => 3),
			CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, -5)
		});
		incident23.AddOption("{=0wRiev5G}Show genuine remorse and participate in the burial rites for the deer, just in case, even if your troops make comments behind your back", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 100),
			CampaignIncidentEffects.MoraleChange(-5f),
			CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, 20)
		});
		...
		incident24.AddOption("{=Edn4syjZ}Indulge your men, even though you have little doubt that the wedding is an invention", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100),
			CampaignIncidentEffects.MoraleChange(5f),
			CampaignIncidentEffects.ChangeItemAmount(() => Game.Current.ObjectManager.GetObject<ItemObject>("wine"), () => -1)
		});
		...
		incident25.AddOption("{=bi8UfCdg}Indulge your man, and wait for his to complete his errand", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, -10),
			CampaignIncidentEffects.GoldChange(() => 1000).WithChance(0.01f)
		});
		...
		Incident incident26 = RegisterIncident("incident_solider_is_wanted_criminal", "{=eN5vqUuF}Soldier is Wanted Criminal", "{=z4RgNBkq}On your way out of the village, a delegation led by the elder approaches you. They point to a {TROOP_TYPE} in your ranks - a good warrior, but none too outgoing. They say he used to farm in this village, but fled after he killed his neighbor in a quarrel over boundary stones. The accused snaps back with a sudden vehemence - his victim was a cheat, and deserved what he got. Under the law, murder requires the death penalty, though custom allows blood money to be paid instead.", "LeavingVillage", "TroopSettlementRelation", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			CharacterObject wantedCriminalTroop2 = GetWantedCriminalTroop();
			if (wantedCriminalTroop2 == null)
			{
				return false;
			}
			description.SetTextVariable("TROOP_TYPE", wantedCriminalTroop2.Name);
		...
		incident26.AddOption("{=HXj3itpQ}Your {TROOP_TYPE} confessed. There is no option but to let the law take its course", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100),
			CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => x.Character == GetWantedCriminalTroop(), () => 1),
			CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, 10)
		}, delegate(TextObject text)
		{
			CharacterObject wantedCriminalTroop = GetWantedCriminalTroop();
			text.SetTextVariable("TROOP_TYPE", wantedCriminalTroop.Name);
			return true;
		});
		...
		{
			CharacterObject soldierInDebtTroop = GetSoldierInDebtTroop();
			if (soldierInDebtTroop == null)
			{
				return false;
			}
			description.SetTextVariable("TROOP_TYPE", soldierInDebtTroop.Name);
		...
		incident29.AddOption("{=xSoKTeLL}Allow your men to collect some berries from the bushes", new List<IncidentEffect> { CampaignIncidentEffects.Select(CampaignIncidentEffects.Group(CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100), CampaignIncidentEffects.MoraleChange(5f)), CampaignIncidentEffects.WoundTroopsRandomly(3), 0.2f) });
		...
		incident30.AddOption("{=E659QFHp}Free meat, a bit of fun and practice for your men. Sound the hunting horn.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.ChangeItemAmount(() => Game.Current.ObjectManager.GetObject<ItemObject>("meat"), () => 3),
			CampaignIncidentEffects.PartyExperienceChance(200),
			CampaignIncidentEffects.WoundTroopsRandomly(2).WithChance(0.5f)
		});
		...
		{
			CampaignIncidentEffects.ChangeItemAmount(() => Game.Current.ObjectManager.GetObject<ItemObject>("meat"), () => 2),
			CampaignIncidentEffects.DisorganizeParty()
		});
		incident30.AddOption("{=5rjUJaW4}Hunting requires patience. You've got real battles to fight.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 100) });
		...
		incident31.AddOption("{=chZrxxe1}The hunt is on! Surround the thicket with spearmen and drive the boar out", new List<IncidentEffect> { CampaignIncidentEffects.Select(CampaignIncidentEffects.Group(CampaignIncidentEffects.MoraleChange(5f), CampaignIncidentEffects.ChangeItemAmount(() => Game.Current.ObjectManager.GetObject<ItemObject>("meat"), () => 3), CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100)), CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => true, () => 1), 0.75f) });
		incident31.AddOption("{=Hl2EsIob}Leave the animal be", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 100) });
		Incident incident32 = RegisterIncident("incident_broken_wagon", "{=8FobMTCH}Broken-down Wagon", "{=FJzoOKIU}You come across a trader whose wagon has gotten stuck in the road after his draught horse went lame. He is desperate to sell his wares before they spoil, and offers them at a bargain price.", "LeavingEncounter", "TroopSettlementRelation", CampaignTime.Days(60f), (TextObject description) => !MobileParty.MainParty.IsCurrentlyAtSea && MobileParty.MainParty.ItemRoster.Any((ItemRosterElement x) => x.EquipmentElement.Item.HasHorseComponent || x.EquipmentElement.Item.StringId == "mule"));
		incident32.AddOption("{=ywaqOBG6}Give some draught animal to him so that he can continue on his way.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 200),
			CampaignIncidentEffects.ChangeItemAmount(GetBrokenWagonDraughtAnimalItem, () => -1)
		});
		incident32.AddOption("{=1bobtmxl}Sell him your cheapest horse so that he can continue.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100),
			CampaignIncidentEffects.GoldChange(() => 200),
			CampaignIncidentEffects.ChangeItemAmount(GetBrokenWagonCheapestHorseItem, () => -1)
		});
		incident32.AddOption("{=buCYa33Q}Buy his wares at bargain price.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100),
			CampaignIncidentEffects.ChangeItemAmount(() => Game.Current.ObjectManager.GetObject<ItemObject>("cotton"), () => 1),
			CampaignIncidentEffects.ChangeItemAmount(() => Game.Current.ObjectManager.GetObject<ItemObject>("wine"), () => 1),
			CampaignIncidentEffects.GoldChange(() => -100)
		});
		incident32.AddOption("{=wIakrAb}Explain to him that you cannot help.", new List<IncidentEffect>());
		Incident incident33 = RegisterIncident("incident_honor_the_slain_foe", "{=bGxsp5Le}Honor the Slain Foe", "{=lFaBQPxk}Your {CULTURE_NAME} troops ask if they can take charge of the {CULTURE_BURIAL_STYLE} of the {CULTURE_NAME} in the enemy's ranks, declaring that they feel a duty to their kin even if they fought in a bad cause.", "LeavingBattle", "PostBattle", CampaignTime.Days(60f), delegate(TextObject description)
		{
			TroopRosterElement honorTheSlainFoeRandomTroop = GetHonorTheSlainFoeRandomTroop();
			if (honorTheSlainFoeRandomTroop.Character == null)
			{
				return false;
			}
			description.SetTextVariable("CULTURE_NAME", honorTheSlainFoeRandomTroop.Character.Culture.Name);
			switch (honorTheSlainFoeRandomTroop.Character.Culture.StringId)
			{
			case "nord":
			case "battania":
			case "khuzait":
		...
		incident36.AddOption("{=D750cNUH}Turn a blind eye, even though the practice is considered little better than counterfeiting", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, -100) });
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 100),
			CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => !x.Character.IsHero, () => (int)(0.05f * (float)MobileParty.MainParty.MemberRoster.TotalRegulars)).WithChance(0.5f)
		});
		...
		{
			Village seededRandomElement2 = IncidentHelper.GetSeededRandomElement(Village.All.Where((Village x) => x.VillageType == DefaultVillageTypes.VineYard).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
			if (seededRandomElement2 == null)
			{
				return false;
			}
			description.SetTextVariable("WINE_SETTLEMENT_NAME", seededRandomElement2.Name);
		...
		{
			CharacterObject noMoodForMercyRandomTroop2 = GetNoMoodForMercyRandomTroop();
			if (noMoodForMercyRandomTroop2 == null)
			{
				return false;
			}
			description.SetTextVariable("TROOP_TYPE", noMoodForMercyRandomTroop2.Name);
			PlayerBattleEndedLogEntry playerBattleEndedLogEntry3 = Campaign.Current.LogEntryHistory.FindLastGameActionLog((PlayerBattleEndedLogEntry x) => true);
			return playerBattleEndedLogEntry3 != null && playerBattleEndedLogEntry3.PlayerCasualties > 0;
		});
		...
		{
			CharacterObject noMoodForMercyRandomTroop = GetNoMoodForMercyRandomTroop();
			text.SetTextVariable("TROOP_TYPE", noMoodForMercyRandomTroop.Name);
			return true;
		});
		incident41.AddOption("{=mBkCqpEj}Tell your men that they can avenge their comrade as they see fit", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100),
			CampaignIncidentEffects.MoraleChange(10f),
			CampaignIncidentEffects.RemovePrisonersRandomlyWithPredicate((TroopRosterElement x) => true, 3)
		});
		Incident incident42 = RegisterIncident("incident_misplaced_vengeance", "{=fiesVorG}Misplaced Vengeance", "{=i0SwEBdf}Throughout your visit to {VILLAGE}, the villagers were worried that the enemy who sacked {LOOTED_VILLAGE} might be sending patrols their way. As you leave, you find out that their fears were not misplaced. The first thing you see is a flight of ravens in the sky. Your men begin to mutter to themselves, but soon they stumble across a sight that silences them - a pile of burned corpses, stacked by a crossroad, left by the {ENEMY} as their calling card. You press on, but you soon notice that your men are casting an eye towards your prisoners. They may not have been directly involved in what happened in {LOOTED_VILLAGE}, but you doubt your men care much.", "LeavingVillage", "PostBattle", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			Village village5 = MobileParty.MainParty.LastVisitedSettlement.Village;
			Village lootedVillage2 = GetMisplacedVengeanceLootedVillage();
			if (lootedVillage2 == null)
			{
				return false;
			}
		...
		incident42.AddOption("{=SfO3uwdX}Take a brief walk to clear your head, and when you come back to find the prisoners gone, do not ask questions", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100),
			CampaignIncidentEffects.MoraleChange(5f),
			CampaignIncidentEffects.Custom(null, delegate
			{
				Village lootedVillage = GetMisplacedVengeanceLootedVillage();
				CampaignIncidentEffects.RemovePrisonersRandomlyWithPredicate((TroopRosterElement x) => x.Character.Culture == lootedVillage?.Settlement.Culture, 5).Consequence();
			...
			{
				Village misplacedVengeanceLootedVillage = GetMisplacedVengeanceLootedVillage();
				return new IncidentHint(new TextObject("{=gejYla80}Lose {AMOUNT} {?AMOUNT > 1}prisoners{?}prisoner{\\?} of {CULTURE} culture").SetTextVariable("AMOUNT", 5).SetTextVariable("CULTURE", misplacedVengeanceLootedVillage?.Settlement.Culture.Name));
			})
		});
		...
		{
			CharacterObject sleepingSentryTroop = GetSleepingSentryTroop();
			if (sleepingSentryTroop == null)
			{
				return false;
			}
			description.SetTextVariable("RECRUIT", sleepingSentryTroop.Name);
		...
		incident43.AddOption("{=oIknbpZl}Force the recruit to run the gauntlet", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 100),
			CampaignIncidentEffects.MoraleChange(5f),
			CampaignIncidentEffects.Select(CampaignIncidentEffects.WoundTroop(GetSleepingSentryTroop, 1), CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => x.Character.Tier <= 2, () => 1), 0.5f)
		});
		...
		{
			ItemRosterElement tradeProposalGood5 = GetTradeProposalGood();
			if (tradeProposalGood5.EquipmentElement.Item == null)
			{
				return false;
			}
			description.SetTextVariable("GOOD", tradeProposalGood5.EquipmentElement.Item.Name);
		...
		incident44.AddOption("{=b9S5bNbq}As a merchant he should know his trade. Keep silent and take the offer", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.Custom(null, delegate
			{
				ItemRosterElement tradeProposalGood4 = GetTradeProposalGood();
				MobileParty.MainParty.ItemRoster.AddToCounts(tradeProposalGood4.EquipmentElement, -1);
				GiveGoldAction.ApplyBetweenCharacters(null, Hero.MainHero, tradeProposalGood4.EquipmentElement.ItemValue * 2);
				TextObject textObject8 = new TextObject("{=JmG1KrTw}Lost 1 {GOOD} and gained {AMOUNT}{GOLD_ICON}");
				textObject8.SetTextVariable("GOOD", tradeProposalGood4.EquipmentElement.Item.Name);
				textObject8.SetTextVariable("AMOUNT", tradeProposalGood4.EquipmentElement.ItemValue * 2);
				return new List<TextObject> { textObject8 };
			}, delegate
			{
				ItemRosterElement tradeProposalGood3 = GetTradeProposalGood();
				TextObject textObject7 = new TextObject("{=baqpnA0F}Lose 1 {GOOD}, gain {AMOUNT}{GOLD_ICON}");
				textObject7.SetTextVariable("GOOD", tradeProposalGood3.EquipmentElement.Item.Name);
				textObject7.SetTextVariable("AMOUNT", tradeProposalGood3.EquipmentElement.ItemValue * 2);
				return new IncidentHint(textObject7);
			})
		});
		...
		incident44.AddOption("{=QWHwvRtC}As a fool and his money are soon parted, you'll do him a favor by saving him some time. Haggle for more", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.Custom(null, delegate
			{
				ItemRosterElement tradeProposalGood2 = GetTradeProposalGood();
				MobileParty.MainParty.ItemRoster.AddToCounts(tradeProposalGood2.EquipmentElement, -1);
				GiveGoldAction.ApplyBetweenCharacters(null, Hero.MainHero, tradeProposalGood2.EquipmentElement.ItemValue * 3);
				TextObject textObject6 = new TextObject("{=JmG1KrTw}Lost 1 {GOOD} and gained {AMOUNT}{GOLD_ICON}");
				textObject6.SetTextVariable("GOOD", tradeProposalGood2.EquipmentElement.Item.Name);
				textObject6.SetTextVariable("AMOUNT", tradeProposalGood2.EquipmentElement.ItemValue * 3);
				return new List<TextObject> { textObject6 };
			}, delegate
			{
				ItemRosterElement tradeProposalGood = GetTradeProposalGood();
				TextObject textObject5 = new TextObject("{=baqpnA0F}Lose 1 {GOOD}, gain {AMOUNT}{GOLD_ICON}");
				textObject5.SetTextVariable("GOOD", tradeProposalGood.EquipmentElement.Item.Name);
				textObject5.SetTextVariable("AMOUNT", tradeProposalGood.EquipmentElement.ItemValue * 3);
				return new IncidentHint(textObject5);
			})
		});
		...
		incident45.AddOption("{=AxGaZNry}Let him do it and reward him generously, but tell him to describe your losses and faults as well", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.InfluenceChange(-100f)
		});
		incident45.AddOption("{=m35ezdZG}You don't need a bard to recount your heroic deeds", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100) });
		incident45.AddOption("{=bJBLAnrc}Shower him with gold and tell him that he should feel free to embellish your story as he sees fit", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.GoldChange(() => -1000),
			CampaignIncidentEffects.RenownChange(20f),
			CampaignIncidentEffects.InfluenceChange(200f)
		});
		incident45.AddOption("{=HbOFGhTG}Tragic tales are always the most popular. Ensure the bard does not leave out the daemon who sits on your shoulder, your bouts of madness, and the witch who foretold your terrible but heroic death", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.RenownChange(30f),
			CampaignIncidentEffects.GoldChange(() => -200)
		});
		...
		{
			CharacterObject character3 = IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.MemberRoster.GetTroopRoster().Where(LoveMarriageTroopPredicate).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
			if (character3 == null)
			{
				return false;
			}
			description.SetTextVariable("KNIGHT", character3.Name);
		...
		incident46.AddOption("{=uppCchb0}Let them marry. Love must triumph over a father's pride", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 200),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100),
			CampaignIncidentEffects.KillTroopsRandomly(LoveMarriageTroopPredicate, () => 1).WithChance(0.5f)
		});
		...
		incident47.AddOption("{=xhuGmF9r}That the force of the blade lies only in the hand that wields it.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 100) });
		...
		incident48.AddOption("{=sxH5NHba}\"I have, and it is but an appetite-whetter before the main meal.\"", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100),
			CampaignIncidentEffects.RenownChange(20f),
			CampaignIncidentEffects.HeroRelationChange(() => Hero.MainHero.Clan.Kingdom.Leader, -1)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 100),
			CampaignIncidentEffects.HeroRelationChange(() => Hero.MainHero.Clan.Kingdom.Leader, 5)
		}, delegate(TextObject text)
		{
			text.SetTextVariable("KING", Hero.MainHero.Clan.Kingdom.Leader.Name);
			return true;
		});
		incident48.AddOption("{=naVH8uSO}\"Any blood I have shed may have been necessary but I hope that Heaven will forgive me.\"", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 100) });
		...
		{
			Hero intriguingRumorsRivalLord4 = GetIntriguingRumorsRivalLord();
			if (intriguingRumorsRivalLord4 == null)
			{
				return false;
			}
			description.SetCharacterProperties("RIVALLORD", intriguingRumorsRivalLord4.CharacterObject);
			return true;
		});
		incident49.AddOption("{=Gjd6nP29}You will not sully the good name of anyone with an obviously false rumor", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, 100) });
		incident49.AddOption("{=jE9aKaYd}People will believe a noble's former servant. This man's favor is worth a purse of silver to you.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, -100),
			CampaignIncidentEffects.GoldChange(() => -200),
			CampaignIncidentEffects.InfluenceChange(10f),
			CampaignIncidentEffects.Custom(null, delegate
			{
				Hero intriguingRumorsRivalLord3 = GetIntriguingRumorsRivalLord();
				if (intriguingRumorsRivalLord3 != null)
				{
					ChangeRelationAction.ApplyRelationChangeBetweenHeroes(Hero.MainHero, intriguingRumorsRivalLord3, -5);
					intriguingRumorsRivalLord3.AddInfluenceWithKingdom(-10f);
					TextObject textObject4 = new TextObject("{=WkIylUOa}{RIVALLORD.NAME} lost {AMOUNT} influence.");
					textObject4.SetCharacterProperties("RIVALLORD", intriguingRumorsRivalLord3.CharacterObject);
					textObject4.SetTextVariable("AMOUNT", 10);
					return new List<TextObject> { textObject4 };
				}
				return new List<TextObject>();
			}, delegate
			{
				Hero intriguingRumorsRivalLord2 = GetIntriguingRumorsRivalLord();
				TextObject textObject3 = new TextObject("{=NQKDUIGg}{RIVALLORD.NAME} loses {AMOUNT} influence");
				textObject3.SetCharacterProperties("RIVALLORD", intriguingRumorsRivalLord2.CharacterObject);
				textObject3.SetTextVariable("AMOUNT", 10);
				return new IncidentHint(textObject3);
			})
		});
		incident49.AddOption("{=bZUMOAjU}Crack a few jokes to your men at {RIVALLORD.NAME}'s expense, but otherwise do nothing", new List<IncidentEffect> { CampaignIncidentEffects.MoraleChange(5f) }, delegate(TextObject text)
		{
			Hero intriguingRumorsRivalLord = GetIntriguingRumorsRivalLord();
			text.SetCharacterProperties("RIVALLORD", intriguingRumorsRivalLord.CharacterObject);
			return true;
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100),
			CampaignIncidentEffects.ChangeTroopAmount(() => MobileParty.MainParty.LastVisitedSettlement.Culture.BasicTroop, 4)
		});
		...
		{
			CharacterObject regularTroop2 = GetRegularTroop();
			if (regularTroop2 == null)
			{
				return false;
			}
			CharacterObject nobleTroop = GetNobleTroop(regularTroop2.Culture);
			if (nobleTroop == null)
			{
				return false;
			}
			description.SetTextVariable("TROOPS", regularTroop2.Name);
			description.SetTextVariable("NOBLE_TIER", nobleTroop.Name);
			return true;
		});
		incident53.AddOption("{=HpAbeiKP}Courage and skill deserve a reward, even if the result is not what you get", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 200),
			CampaignIncidentEffects.MoraleChange(-10f),
			CampaignIncidentEffects.UpgradeTroop(GetRegularTroop, GetNobleTroopForUpgrade, 1, () => Campaign.Current.IncidentManager.ActiveIncidentSeed)
		});
		incident53.AddOption("{=oL1qlaxH}Explain to him your reservations, even though you can tell he won't like to hear them, and let him make up his own mind", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, 100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100),
			CampaignIncidentEffects.Select(CampaignIncidentEffects.Group(CampaignIncidentEffects.MoraleChange(-5f), CampaignIncidentEffects.UpgradeTroop(GetRegularTroop, GetNobleTroopForUpgrade, 1, () => Campaign.Current.IncidentManager.ActiveIncidentSeed)), CampaignIncidentEffects.Custom(null, () => new List<TextObject>(), (IncidentEffect effect) => new IncidentHint(new TextObject("{=lobJVVWT}Nothing happens"))), 0.5f)
		});
		...
		incident54.AddOption("{=QDbl4lB7}Release your captive, even if some will mutter that you have a soft heart and a soft head", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 200),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100),
			CampaignIncidentEffects.RemovePrisonersRandomlyWithPredicate(BanditPrisonerPredicate, 1)
		});
		...
		Incident incident55 = RegisterIncident("incident_turn_pike", "{=kl13BJZB}Turnpike", "{=VauaVUNS}You notice a footpath leading through your estate which could be quickly improved to be used by wagons and be a shortcut between the main trade roads. You could charge tolls and turn it into a turnpike, though the locals would resent the increased traffic and fees on their customary route.", new string[2] { "LeavingCastle", "LeavingTown" }, "FiefManagement", CampaignTime.Years(1000f), delegate
		{
			...
			Building turnPikeBuilding = GetTurnPikeBuilding();
			return turnPikeBuilding != null && turnPikeBuilding.CurrentLevel == turnPikeBuilding.BuildingType.StartLevel;
		});
		incident55.AddOption("{=aF4Q5TJc}Widen the road for wagons but charge tolls to use it", new List<IncidentEffect>
		{
			CampaignIncidentEffects.BuildingLevelChange(GetTurnPikeBuilding, () => 1),
			CampaignIncidentEffects.TownBoundVillageRelationChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, -10),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100)
		});
		...
		Incident incident56 = RegisterIncident("incident_endebted_farmers", "{=Vbs9aocS}Endebted Farmers", "{=cRMxKTa8}Several freeholding farmers in your fief have fallen into debt through what appears to be a combination of bad luck and risky investments - a plough horse that died, a crop ruined by frost. They are unable to pay their taxes, and ask for a reprieve for several years.", new string[2] { "LeavingCastle", "LeavingTown" }, "FiefManagement", CampaignTime.Years(1000f), delegate
		{
			if (MobileParty.MainParty.LastVisitedSettlement == null || !MobileParty.MainParty.LastVisitedSettlement.IsFortification || MobileParty.MainParty.LastVisitedSettlement.OwnerClan != Clan.PlayerClan)
			{
				return false;
			}
			Building endebtedFarmersBuilding = GetEndebtedFarmersBuilding();
			return endebtedFarmersBuilding != null && endebtedFarmersBuilding.CurrentLevel == endebtedFarmersBuilding.BuildingType.StartLevel;
		});
		...
		{
			CampaignIncidentEffects.BuildingLevelChange(GetEndebtedFarmersBuilding, () => 1),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 100)
		});
		incident56.AddOption("{=jeWayyGY}A reprieve is fine, but you tack on a bit of interest so that you're not overwhelmed with hard luck stories", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100) });
		...
		{
			CampaignIncidentEffects.GoldChange(() => 1000),
			CampaignIncidentEffects.InfluenceChange(-20f)
		});
		...
		Incident incident57 = RegisterIncident("incident_fertility_festival", "{=o5LpNEIa}Fertility Festival", "{=lfJ6OHgX}The village has seen an unexpected rise in pregnancies. Some of your villagers ask your permission to hold a festival in honor of Agalea, protectress of motherhood. Agalea has never been recognized as a saint by the imperial Senate, and many consider her veneration to be a primitive Palaic holder. Her festivals tend to be rather raucous, befitting the theme of fertility, and also rather expensive. It would certainly set tongues wagging in the district. Do you grant permission?", "LeavingVillage", "FiefManagement", CampaignTime.Years(1000f), delegate
		{
			if (MobileParty.MainParty.LastVisitedSettlement == null)
			{
				return false;
		...
		incident57.AddOption("{=EFUNx0cv}A vat of wine for you to dance in honor of the lady Agalea!", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, 10),
			CampaignIncidentEffects.InfluenceChange(-100f),
			CampaignIncidentEffects.VillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Village, 20),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.LastVisitedSettlement.Village.Bound.Town, -10)
		});
		incident57.AddOption("{=NWNIVZF0}Give the villagers a stern lecture on the importance of sound doctrine and lawful worship", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, -10),
			CampaignIncidentEffects.InfluenceChange(100f),
			CampaignIncidentEffects.VillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Village, 5)
		});
		incident57.AddOption("{=J9BsFeSS}Tell the villagers to keep things discrete", new List<IncidentEffect>
		{
			CampaignIncidentEffects.VillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Village, 10),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.LastVisitedSettlement.Village.Bound.Town, -5)
		});
		...
		incident58.AddOption("{=8JqCDWRd}Welcome them, and give them a donation to purchase small plots of land.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.VillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Village, 20),
			CampaignIncidentEffects.TownSecurityChange(() => MobileParty.MainParty.LastVisitedSettlement.Village.Bound.Town, 10).WithChance(0.25f)
		});
		...
		{
			CampaignIncidentEffects.VillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Village, 25),
			CampaignIncidentEffects.TownSecurityChange(() => MobileParty.MainParty.LastVisitedSettlement.Village.Bound.Town, -10).WithChance(0.5f)
		});
		incident58.AddOption("{=o4aQ2UEL}Tell them to make their homes someplace else.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100) });
		Incident incident59 = RegisterIncident("incident_nomads_wish_to_settle", "{=5vN2NZ8R}Nomads Wish to Settle", "{=U4Ybz08z}A desperate band of nomads has arrived at your village, seeking a place to settle down. Their herds were ravaged by an epidemic, they say, and they are ready to take up farming or work as laborers. Otherwise, they must starve or give themselves to another tribe as servants. The villagers are skeptical. Nomads are too lazy to be farmers, they say, preferring horse theft and feuds.", "LeavingVillage", "FiefManagement", CampaignTime.Years(1000f), delegate
		{
			...
			Village village4 = MobileParty.MainParty.LastVisitedSettlement.Village;
			bool num13 = new string[3] { "khuzait", "aserai", "empire_s" }.Contains(village4.Settlement.Culture.StringId);
			bool flag4 = Hero.MainHero.Clan.Fiefs.Contains(village4.Bound.Town);
			return num13 && flag4;
		});
		...
		incident59.AddOption("{=jUlHTiSU}Agree with the villagers that nomads are more trouble than they are worth.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100) });
		Incident incident60 = RegisterIncident("incident_conflict_over_commons_both_yours", "{=uqB1fKFY}Conflict Over Commons", "{=raLLq10c}Grazing land has grown scarce near {TOWN_OR_CASTLE_NAME}, and a heath between {POORER_VILLAGE} and {RICHER_VILLAGE}, covered with thistle and formerly of little value, has become a point of contention. {POORER_VILLAGE} claims that they have been taking goats there for generations, but {RICHER_VILLAGE} has produced a document showing legal title.", new string[2] { "LeavingCastle", "LeavingTown" }, "FiefManagement", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			Village village2 = source.LastOrDefault();
			Village village3 = source.FirstOrDefault();
			if (village2 == null || village3 == null || village2 == village3)
			{
				return false;
			}
			float num12 = Village.All.Sum((Village x) => x.Hearth) / (float)Village.All.Count;
			if (village2.Hearth < num12 || village3.Hearth < num12)
			{
				return false;
			}
			description.SetTextVariable("TOWN_OR_CASTLE_NAME", MobileParty.MainParty.LastVisitedSettlement.Name);
			description.SetTextVariable("RICHER_VILLAGE", village2.Name);
			description.SetTextVariable("POORER_VILLAGE", village3.Name);
			return true;
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, 200),
			CampaignIncidentEffects.VillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Town.Villages.OrderBy((Village x) => x.Hearth).LastOrDefault(), 5),
			CampaignIncidentEffects.VillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Town.Villages.OrderBy((Village x) => x.Hearth).FirstOrDefault(), -10)
		});
		...
		incident60.AddOption("{=wlVCpQHS}Have them split it.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.VillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Town.Villages.OrderBy((Village x) => x.Hearth).FirstOrDefault(), 5)
		});
		...
		{
			CampaignIncidentEffects.SettlementRelationChange(() => GetOtherVillage().Settlement, -10),
			CampaignIncidentEffects.HeroRelationChange(() => GetOtherVillage().Settlement.OwnerClan.Leader, -10),
			CampaignIncidentEffects.VillageHearthChange(GetVillageOne, -5).WithChance(0.25f),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, 20),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 100)
		});
		...
		{
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, 5),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, -100),
			CampaignIncidentEffects.HeroRelationChange(() => GetOtherVillage().Settlement.OwnerClan.Leader, 20)
		}, delegate(TextObject text)
		{
			Village otherVillage = GetOtherVillage();
			text.SetCharacterProperties("NOBLE", otherVillage.Settlement.OwnerClan.Leader.CharacterObject);
			return true;
		});
		...
		{
			CampaignIncidentEffects.TownSecurityChange(() => MobileParty.MainParty.LastVisitedSettlement.Village.Bound.Town, -5),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.LastVisitedSettlement.Village.Bound.Town, 10),
			CampaignIncidentEffects.InfluenceChange(20f)
		});
		incident62.AddOption("{=zEPpgYaf}Reading won't help a farmboy plough, but might goad him to do foolish things that get him killed", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 100) });
		...
		{
			CampaignIncidentEffects.VillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Village, 20),
			CampaignIncidentEffects.TownSecurityChange(() => MobileParty.MainParty.LastVisitedSettlement.Village.Bound.Town, -10),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 100),
			CampaignIncidentEffects.InfestNearbyHideout(() => MobileParty.MainParty.LastVisitedSettlement)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 200),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.LastVisitedSettlement.Village.Bound.Town, 100),
			CampaignIncidentEffects.TownSecurityChange(() => MobileParty.MainParty.LastVisitedSettlement.Village.Bound.Town, -5)
		});
		Incident incident64 = RegisterIncident("incident_wages_of_war_and_diseases", "{=7q53TLg7}Wages of war and diseases", "{=FZXYHC7s}Between the wars and an epidemic, the villages near {FIEF_NAME} are experiencing a shortage of people, especially young men. Fields are going fallow for lack of hands to plough them, especially those cleared only in recent generations where the title is in dispute. As liege, you can redistribute this unused land, but you will need to find labor.", new string[2] { "LeavingCastle", "LeavingTown" }, "FiefManagement", CampaignTime.Days(60f), delegate(TextObject description)
		{
			if (MobileParty.MainParty.LastVisitedSettlement == null || !MobileParty.MainParty.LastVisitedSettlement.IsFortification || MobileParty.MainParty.LastVisitedSettlement.OwnerClan != Clan.PlayerClan)
			{
				return false;
			}
			Building wagesOfWarAndDiseasesBuilding = GetWagesOfWarAndDiseasesBuilding();
			if (wagesOfWarAndDiseasesBuilding == null)
			{
				return false;
			}
			float num11 = Village.All.Average((Village x) => x.Hearth);
			if (MobileParty.MainParty.LastVisitedSettlement.BoundVillages.Average((Village x) => x.Hearth) > num11)
			{
				return false;
			}
			description.SetTextVariable("FIEF_NAME", MobileParty.MainParty.LastVisitedSettlement.Name);
			return wagesOfWarAndDiseasesBuilding.CurrentLevel == wagesOfWarAndDiseasesBuilding.BuildingType.StartLevel;
		});
		...
		{
			CampaignIncidentEffects.BuildingLevelChange(GetWagesOfWarAndDiseasesBuilding, () => 1),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100),
			CampaignIncidentEffects.TownBoundVillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, 10)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 200),
			CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => !x.Character.IsHero, () => 10),
			CampaignIncidentEffects.TownBoundVillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, 20)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 200),
			CampaignIncidentEffects.TownBoundVillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, 30),
			CampaignIncidentEffects.GoldChange(() => -500),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, 10)
		});
		Incident incident65 = RegisterIncident("incident_successful_harvest", "{=MaaFv4Po}Successful Harvest", "{=evcKUaab}An early thaw and a warm spring has ensured a bumper crop, which the prayers of the villagers and the efforts of the local cats have protected from rats and birds. Such bounties occur rarely. What will you do?", new string[2] { "LeavingTown", "LeavingCastle" }, "FiefManagement", CampaignTime.Days(60f), delegate
		{
			if (MobileParty.MainParty.LastVisitedSettlement == null)
			{
				return false;
		...
		{
			CampaignIncidentEffects.MoraleChange(10f),
			CampaignIncidentEffects.TownBoundVillageRelationChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, 5),
			CampaignIncidentEffects.TownBoundVillageHearthChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, 10),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, 5),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 200),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100)
		...
		incident66.AddOption("{=B1cyEaQA}Dreams are but a temporary madness. Heaven does not speak to us in this way.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 100) });
		Incident incident67 = RegisterIncident("incident_market_manipulation", "{=tGwbM59L}Market Manipulation", "{=5OuDl2Po}As you come into town, the merchant {MERCHANT_NAME} makes you an offer. Local merchants are on edge awaiting an incoming caravan from {PLACE}. If you claim that you found its wreckage, looted by bandits, the prices of goods would rise considerably. He offers to pay you 1000 denars, in addition to whatever extra you can make by selling your own wares once the prices rise.", "EnteringTown", "Profit", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			Town randomElement2 = Town.AllTowns.GetRandomElement();
			Hero marketManipulationMerchant2 = GetMarketManipulationMerchant();
			if (marketManipulationMerchant2 == null)
			{
				return false;
			}
			description.SetTextVariable("MERCHANT_NAME", marketManipulationMerchant2.Name);
			description.SetTextVariable("PLACE", randomElement2.Name);
		...
		incident67.AddOption("{=SLASexva}Accept, but leave enough room for doubt in your account that it won't look too bad if the caravan turns up.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, -100),
			CampaignIncidentEffects.GoldChange(() => 1000),
			CampaignIncidentEffects.SkillChange(DefaultSkills.Trade, 50f),
			CampaignIncidentEffects.Custom(null, delegate
			{
				...
				{
					marketData2.SetDemand(item2, marketData2.GetDemand(item2) * 1.1f);
				}
			...
			}, (IncidentEffect effect) => new IncidentHint(new TextObject("{=uiomHbfS}Raise demand of trade goods by {AMOUNT}% in {TOWN}").SetTextVariable("AMOUNT", 10).SetTextVariable("TOWN", Settlement.CurrentSettlement.Name)))
		});
		incident67.AddOption("{=EyHD7gjy}Say you'll swear up and down that you saw the wreckage, damaging your reputation, but demand 2000 denars instead.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.SkillChange(DefaultSkills.Trade, 100f),
			CampaignIncidentEffects.Custom(null, delegate
			{
		...
		incident67.AddOption("{=aMMxVIpa}Erupt in anger for all to see, and curse {MERCHANT} for taking you for a base liar.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, 100),
			CampaignIncidentEffects.SettlementRelationChange(() => Settlement.CurrentSettlement, 10),
			CampaignIncidentEffects.HeroRelationChange(GetMarketManipulationMerchant, -20)
		}, delegate(TextObject text)
		{
			Hero marketManipulationMerchant = GetMarketManipulationMerchant();
			text.SetTextVariable("MERCHANT", marketManipulationMerchant.Name);
			return true;
		});
		...
		Incident incident68 = RegisterIncident("incident_gnostic_boys", "{=hewpozXg}Gnostic Boys", "{=H7Co1wqC}Two of your {NOBLE_TIER} have lately been causing a stir in the camp with their irreverent religious preaching. They propound that this world is a illusion, created by a being called the Demiurge for its own amusement, and humans are but game-pieces. You think this may be the Marcorionist doctrine, declared heretical by the imperial Senate some years ago. It is clearly disturbing your more traditional-minded troops and is probably also not good for your party's reputation.", "LeavingVillage", "DreamsSongsAndSigns", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			CharacterObject gnosticBoysNobleTroop = GetGnosticBoysNobleTroop();
			if (gnosticBoysNobleTroop == null)
			{
				return false;
			}
			description.SetTextVariable("NOBLE_TIER", gnosticBoysNobleTroop.Name);
			return true;
		});
		incident68.AddOption("{=WyuxrGqC}Tell them, in a towering fury, that if they do not wish to be burned then they had best either leave your party or cease this sacrilege", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.ChangeTroopAmount(GetGnosticBoysNobleTroop, -2)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 100),
			CampaignIncidentEffects.MoraleChange(-20f).WithChance(0.5f)
		}, delegate(TextObject text)
		{
		...
		Incident incident69 = RegisterIncident("incident_speculative_investment", "{=12bmA5qD}Speculative Investment", "{=vPNDzUAF}You strike up a conversation with a trader in town, who is desperate for news of the wars. When you tell him about the looting of {VILLAGE_NAME}, he makes a proposition. The price of grain will almost certainly rise, and he intends to buy up food now to sell it later when the hunger hits. He needs money, and he will sell you {OTHER_GOOD} at far below their market value - 1000 denars. He asks you also to keep this information to yourself.", "LeavingTown", "Profit", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			ItemObject speculativeInvestmentOtherGood2 = GetSpeculativeInvestmentOtherGood();
			description.SetTextVariable("OTHER_GOOD", speculativeInvestmentOtherGood2.Name);
			return Hero.MainHero.GetSkillValue(DefaultSkills.Trade) >= 20;
		});
		incident69.AddOption("{=aUui6FYp}All's fair in trade and war. You accept the deal.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 50),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -50),
			CampaignIncidentEffects.GoldChange(() => -500),
			CampaignIncidentEffects.ChangeItemAmount(GetSpeculativeInvestmentOtherGood, delegate
			{
				ItemObject speculativeInvestmentOtherGood = GetSpeculativeInvestmentOtherGood();
				return (int)Math.Round(1000f / (float)speculativeInvestmentOtherGood.Value);
			})
		});
		incident69.AddOption("{=2xvqOUJh}Tell him quietly that you will not profit off of the misery of others.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 100) });
		...
		incident70.AddOption("{=bbH4PqtD}Let the tales spread! Sanity and caution do not inspire legend", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.RenownChange(10f),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 100),
			CampaignIncidentEffects.ChangeTroopAmount(GetMadDogTroop, 1)
		});
		...
		Incident incident71 = RegisterIncident("incident_besiegers_blues", "{=uTsdJCSU}Besiegers' Blues", "{=IxgJotDk}At first this siege gave your men a welcome respite from the hardships of the march, but now inactivity and boredom has started to take a toll. Even dice and gossip have lost their charms, and you're starting to see quarrels and lapses in discipline.", "DuringSiege", "Siege", CampaignTime.Years(1000f), (TextObject description) => MobileParty.MainParty.MemberRoster.TotalRegulars >= 40 && PlayerSiege.PlayerSiegeEvent?.BesiegerCamp.SiegeEngines?.SiegePreparations != null);
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100),
			CampaignIncidentEffects.SiegeProgressChange(() => PlayerSiege.PlayerSiegeEvent.BesiegerCamp.SiegeEngines.SiegePreparations.Progress * 0.2f)
		});
		...
		{
			CampaignIncidentEffects.ChangeTroopAmount(GetSkolderbrotvaTroop, 2),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -200),
			CampaignIncidentEffects.MoraleChange(-5f)
		});
		incident72.AddOption("{=78VrrDRM}Tell them that you believe them to be oathbreakers, and as such have no place in your ranks.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, 100) });
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 100),
			CampaignIncidentEffects.Group(CampaignIncidentEffects.ChangeTroopAmount(GetSkolderbrotvaTroop, 2), CampaignIncidentEffects.WoundTroop(() => (from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
		...
		Incident incident73 = RegisterIncident("incident_ill_conceived_plans", "{=VSLbywBT}Ill-Conceived Plans", "{=K8DJmY4L}Several of your {TROOP_TYPE} have an idea for the next battle. They suggest that they be hidden in a supply wagon and left for the enemy to discover and drag into camp, whereupon they will leap out and wreak havoc. They are very proud of their scheme, even though you can think of half a dozen reasons it won't work.", "LeavingVillage", "PartyCampLife", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			CharacterObject character2 = IncidentHelper.GetSeededRandomElement((from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
				where !x.Character.IsHero
				select x).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
			if (character2 == null)
			{
				return false;
			}
		...
		incident73.AddOption("{=YhxsQjGM}Nod quickly as though you weren't really listening and move on to some other task.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -50) });
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 50),
			CampaignIncidentEffects.ChangeItemAmount(() => Game.Current.ObjectManager.GetObject<ItemObject>("camel"), () => 4),
			CampaignIncidentEffects.GoldChange(() => -1000)
		});
		incident74.AddOption("{=uD8su80f}Tell them that you need the money for other purposes.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -200) });
		Incident incident75 = RegisterIncident("incident_preemptive_work", "{=Zd3jBBun}Preemptive Work", "{=3IphvZTK}In your absence, several families in your district have been hard at work draining a marsh and turning it into a field. The survey records in your treasury however indicate clearly that the marsh is part of your domain. They almost certainly did not know this, but you are entitled to claim the field or charge them for working it.", new string[2] { "EnteringTown", "EnteringCastle" }, "FiefManagement", CampaignTime.Years(1000f), delegate
		{
			if (MobileParty.MainParty.LastVisitedSettlement == null || !MobileParty.MainParty.LastVisitedSettlement.IsFortification || MobileParty.MainParty.LastVisitedSettlement.OwnerClan != Clan.PlayerClan)
			{
				return false;
			...
			bool flag3 = false;
			for (float num9 = -3f; num9 <= 3f; num9 += 0.25f)
			{
				for (float num10 = -3f; num10 <= 3f; num10 += 0.25f)
				{
					if (num9 * num9 + num10 * num10 <= 9f)
					{
						CampaignVec2 campaignVec3 = lastVisitedSettlement2.Position + new Vec2(num9, num10);
						if (campaignVec3.Face.IsValid() && Campaign.Current.MapSceneWrapper.GetFaceTerrainType(campaignVec3.Face) == TerrainType.Desert)
						{
							flag3 = true;
							break;
						}
			...
			Building preemptiveWorkBuilding = GetPreemptiveWorkBuilding();
			return preemptiveWorkBuilding != null && preemptiveWorkBuilding.CurrentLevel == preemptiveWorkBuilding.BuildingType.StartLevel;
		});
		...
		Incident incident76 = RegisterIncident("incident_through_proper_channels", "{=S12S4pmL}Through the proper channels", "{=3eOOT66E}One of your tenants has a proposal for you. He proposes to dig a channel known as a qanah into a hillside, where the water table is high, which will allow him to flood some low-lying ground and turn it into an oasis garden. The barren hillside is your property, though at present it yields no income of any kind.", new string[2] { "EnteringTown", "EnteringCastle" }, "FiefManagement", CampaignTime.Years(1000f), delegate
		{
			...
			bool flag2 = false;
			for (float num7 = -3f; num7 <= 3f; num7 += 0.25f)
			{
				for (float num8 = -3f; num8 <= 3f; num8 += 0.25f)
				{
					if (num7 * num7 + num8 * num8 <= 9f)
					{
						CampaignVec2 campaignVec2 = currentSettlement2.Position + new Vec2(num7, num8);
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.CurrentSettlement.Town, 20)
		});
		incident76.AddOption("{=iB2g7XBt}Insist he purchase the hillside from your at market rate.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.CurrentSettlement.Town, 15),
			CampaignIncidentEffects.GoldChange(() => 300)
		});
		incident76.AddOption("{=P8W5OQB9}Wait until he digs the channel and clears the fields, then charge him heavily to use the water without which his effort will be for nothing.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -200),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 50),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.CurrentSettlement.Town, 10),
			CampaignIncidentEffects.GoldChange(() => 500)
		});
		Incident incident77 = RegisterIncident("incident_dodgy_spice", "{=5OzX71s3}Dodgy Spice", "{=QEP3ntdb}Merchants are savvy to most attempts to pass off most types of shoddy goods as top quality, but one of your {TROOP.NAME} knows a few tricks that aren't in the usual rogue's repetoire. He suggests buying a selection of spices from the market, then sprinkling a bit of pomegranate rind in with the saffron, some cassia bark with the cinnamon, and a lead compound known as litharge with the pepper.", "LeavingTown", "Profit", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			TroopRosterElement randomTroop = GetRandomTroop();
			if (randomTroop.Character == null)
			{
				return false;
			}
			description.SetCharacterProperties("TROOP", randomTroop.Character);
			return true;
		});
		incident77.AddOption("{=lrysxcAE}A bit of lead never hurt anyone! Try them all, and split the profits with your {TROOP.NAME}.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, -300),
			CampaignIncidentEffects.GoldChange(() => -100),
			CampaignIncidentEffects.ChangeItemAmount(() => Game.Current.ObjectManager.GetObject<ItemObject>("salt"), () => 1)
		}, delegate(TextObject text)
		{
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, -100),
			CampaignIncidentEffects.GoldChange(() => -150),
			CampaignIncidentEffects.ChangeItemAmount(() => Game.Current.ObjectManager.GetObject<ItemObject>("salt"), () => 1)
		});
		...
		Incident incident78 = RegisterIncident("incident_sorrows_of_war", "{=FL8rUSWo}Sorrows of War", "{=a939QfLJ}As you leave {TOWN_NAME}, a young woman throws herself in front of your horse. She raises her face to the sky, tears at her hair, and wails that her young husband signed up to fight for you and was slain in battle. \"You took him from me!\" she screams. It is not clear what she wants.", "LeavingTown", "TroopSettlementRelation", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			PlayerBattleEndedLogEntry playerBattleEndedLogEntry2 = Campaign.Current.LogEntryHistory.FindLastGameActionLog((PlayerBattleEndedLogEntry x) => true);
			if (playerBattleEndedLogEntry2 == null)
			{
				return false;
			}
			if (!playerBattleEndedLogEntry2.HasHeavyCausality)
			{
				return false;
			}
			description.SetTextVariable("TOWN_NAME", MobileParty.MainParty.LastVisitedSettlement.Name);
		...
		incident78.AddOption("{=52l7VuWI}Tell her that Heaven expects her to bear her fate with dignity.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100) });
		...
		incident79.AddOption("{=VHlwd8na}Take a lively interest in each of these ornaments, asking your men at which battle and from which fallen foe they were taken.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.Custom(null, delegate
			{
				List<Settlement> nearbyVillages2 = GetNearbyVillages();
				List<TextObject> list12 = new List<TextObject>();
				foreach (Settlement item4 in nearbyVillages2)
				{
					...
					textObject2.SetTextVariable("AMOUNT", -5);
					textObject2.SetTextVariable("SETTLEMENT", item4.Name);
					list12.Add(textObject2);
				}
				return list12;
			}, (IncidentEffect effect) => new IncidentHint(new TextObject("{=4o7R829M}-5 relations with three surrounding villages")))
		});
		...
		incident79.AddOption("{=SLAFtiET}Pretend you saw nothing.", new List<IncidentEffect> { CampaignIncidentEffects.Custom(null, delegate
		{
			List<Settlement> nearbyVillages = GetNearbyVillages();
			List<TextObject> list11 = new List<TextObject>();
			foreach (Settlement item5 in nearbyVillages)
			{
		...
		incident80.AddOption("{=RPyFwR6I}Erupt in fury, telling the inspector that there is no way you will be fleeced in this way, legally or illegally.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100),
			CampaignIncidentEffects.RenownChange(5f),
			CampaignIncidentEffects.CrimeRatingChange(() => MobileParty.MainParty.CurrentSettlement.MapFaction, -10f)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, 100),
			CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, -20).WithChance(0.5f)
		});
		incident82.AddOption("{=MGfXReQC}Walk up to the merchant, and insist he use the lighter weights to sell you one measure of his goods. Then come back later and return them, insisting he use the heavier weights.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.GoldChange(() => 200),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, -50),
			CampaignIncidentEffects.SkillChange(DefaultSkills.Roguery, 50f)
		});
		...
		{
			TroopRosterElement randomCavalryTroop = GetRandomCavalryTroop();
			if (randomCavalryTroop.Character == null)
			{
				return false;
			}
			description.SetTextVariable("CAVALRYMAN", randomCavalryTroop.Character.Name);
		...
		incident83.AddOption("{=te1A19Xe}Tell your {CAVALRYMAN} that he can sort it out.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100) }, delegate(TextObject text)
		{
		...
		incident84.AddOption("{=57VuY9ln}Poison it! Some women and children of {FORTRESS_NAME} may well perish along with the defenders, but such is war.", new List<IncidentEffect>
		{
			...
			}, (IncidentEffect effect) => new IncidentHint(new TextObject("{=gyRuGBMF}10% of the garrison perishes"))),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.BesiegedSettlement.Town, -20)
		}, delegate(TextObject text)
		{
		...
		incident84.AddOption("{=bGaD3LoG}The best blows are struck not to the body but to the spirit. Let the defenders know you found it by diverting it to a pool where your men can bathe on the hottest, thirstiest days of the siege.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.SiegeProgressChange(() => 0.1f)
		});
		incident84.AddOption("{=iiwhVNBb}You respect the defenders' cunning. Simply observe how they slip out of the fortress, by ropes or hidden gates, and learn from it.", new List<IncidentEffect> { CampaignIncidentEffects.SkillChange(DefaultSkills.Engineering, 100f) });
		...
		{
			CampaignIncidentEffects.GoldChange(() => -500),
			CampaignIncidentEffects.Select(CampaignIncidentEffects.Group(CampaignIncidentEffects.BreachSiegeWall(1), CampaignIncidentEffects.MoraleChange(5f), CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 100), CampaignIncidentEffects.RenownChange(5f), CampaignIncidentEffects.SkillChange(DefaultSkills.Engineering, 50f)), CampaignIncidentEffects.Group(CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, -100), CampaignIncidentEffects.SiegeProgressChange(() => -0.2f)), 0.6f)
		});
		incident85.AddOption("{=kntd1zYo}Tunnel quickly, threatening to execute anyone who refuses for insubordination", new List<IncidentEffect> { CampaignIncidentEffects.Select(CampaignIncidentEffects.Group(CampaignIncidentEffects.SiegeProgressChange(() => 0.2f), CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100), CampaignIncidentEffects.RenownChange(5f), CampaignIncidentEffects.SkillChange(DefaultSkills.Engineering, 50f)), CampaignIncidentEffects.Group(CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100), CampaignIncidentEffects.SiegeProgressChange(() => -0.1f), CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => true, () => 10)), 0.6f) });
		...
		incident86.AddOption("{=PJYgbG2h}\"A purse of silver for every stone brought to me from that wall!\"", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100),
			CampaignIncidentEffects.KillTroopsRandomly((TroopRosterElement x) => true, () => 10),
			CampaignIncidentEffects.GoldChange(() => -500),
			CampaignIncidentEffects.SiegeProgressChange(() => 0.2f).WithChance(0.8f)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100),
			CampaignIncidentEffects.KillTroopsRandomlyOrderedByTier((TroopRosterElement x) => true, () => 10),
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.DemoteTroopsRandomlyWithPredicate((TroopRosterElement x) => !x.Character.IsHero, (CharacterObject x) => true, 2, specifyUnitTypeOnHint: false)
		});
		Incident incident88 = RegisterIncident("incident_the_quiet_life", "{=3M54uDKZ}The Quiet Life", "{=IaOfWrNF}As you pass through your lands, you see several of your {TROOP_NAME} casting envious eyes over the farmers in the fields. They are tired of wandering from place to place, they say, and hope to put down roots somewhere and start a family. Their contract of enlistment is not yet up but it occurs to you that if they settled in your estates, it would be a boon to your local militia.", new string[2] { "EnteringTown", "EnteringCastle" }, "FiefManagement", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			if (MobileParty.MainParty.CurrentSettlement == null)
			{
				return false;
			}
			CharacterObject theQuietLifeTroop = GetTheQuietLifeTroop();
			if (theQuietLifeTroop == null)
			{
				return false;
			}
			description.SetTextVariable("TROOP_NAME", theQuietLifeTroop.Name);
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.ChangeTroopAmount(GetTheQuietLifeTroop, -3),
			CampaignIncidentEffects.SettlementMilitiaChange(() => MobileParty.MainParty.CurrentSettlement, 10)
		}, delegate(TextObject text)
		{
		...
		incident88.AddOption("{=3WFuf7OC}Give them money to purchase land near your fief in {VILLAGE_NAME}, so long as they take charge of training the local militia.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.GoldChange(() => -300),
			CampaignIncidentEffects.ChangeTroopAmount(GetTheQuietLifeTroop, -3),
			CampaignIncidentEffects.SettlementMilitiaChange(() => MobileParty.MainParty.CurrentSettlement, 15)
		}, delegate(TextObject text)
		{
		...
		incident88.AddOption("{=KUb0DRxw}Refuse them permission.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100) });
		Incident incident89 = RegisterIncident("incident_job_offer", "{=xEktDm2z}Job Offer", "{=guEjfYhO}One of your {TROOP_NAME} had some drinks with the arena master in {TOWN} and left the tavern with a job offer - work as a trainer and occasional combatant. The signing bonus is generous. He is reluctant to leave you, and you'd be reluctant to lose a valuable soldier, but he'd be in a fine position to sing your praises.", "LeavingTown", "PartyCampLife", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			CharacterObject jobOfferTroop = GetJobOfferTroop();
			if (jobOfferTroop == null)
			{
				return false;
			}
			description.SetTextVariable("TROOP_NAME", jobOfferTroop.Name);
			description.SetTextVariable("TOWN", MobileParty.MainParty.LastVisitedSettlement.Name);
		...
		incident89.AddOption("{=0awbcZag}Give him your blessing and encourage him to bore everyone to tears with his old war stories.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.RenownChange(10f),
			CampaignIncidentEffects.ChangeTroopAmount(GetJobOfferTroop, -1)
		});
		...
		{
			Workshop secessionOfThePlebsWorkshop = GetSecessionOfThePlebsWorkshop();
			if (secessionOfThePlebsWorkshop == null)
			{
				return false;
			}
			description.SetTextVariable("TOWN_NAME", secessionOfThePlebsWorkshop.Settlement.Name);
			description.SetTextVariable("WORKSHOP", secessionOfThePlebsWorkshop.Name);
			return true;
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 200),
			CampaignIncidentEffects.WorkshopProfitabilityChange(GetSecessionOfThePlebsWorkshop, -0.1f)
		});
		incident90.AddOption("{=zM2vbhrt}Sit down with them and negotiate a mutual understanding.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.WorkshopProfitabilityChange(GetSecessionOfThePlebsWorkshop, -0.05f)
		});
		...
		Incident incident91 = RegisterIncident("incident_occupational_safety", "{=l8rpMldr}Occupational Safety", "{=Eih6lrcN}Business is good at your {WORKSHOP} in {TOWN}, but in the rush to keep up with demand the normal precautions are not being taken. {INJURY_TEXT} He will live, but is unlikely to be able to do skilled work, and the rest of the workers are worried about future accidents.", "EnteringTown", "Workshop", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			Workshop occupationalSafetyWorkshop = GetOccupationalSafetyWorkshop();
			if (occupationalSafetyWorkshop == null)
			{
				return false;
			}
			description.SetTextVariable("TOWN", occupationalSafetyWorkshop.Settlement.Name);
			description.SetTextVariable("WORKSHOP", occupationalSafetyWorkshop.Name);
			string variable = string.Empty;
			if (occupationalSafetyWorkshop.WorkshopType == workshopType)
			{
				variable = "{=JcoSd4oi}One man burned his hand in the oven.";
			}
			else if (occupationalSafetyWorkshop.WorkshopType == workshopType2)
			{
		...
		incident91.AddOption("{=JdlbkM8C}Pay him a pension and order your supervisors to take more care, even if it slows down production.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 100),
			CampaignIncidentEffects.GoldChange(() => -100),
			CampaignIncidentEffects.WorkshopProfitabilityChange(GetOccupationalSafetyWorkshop, -0.1f)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100),
			CampaignIncidentEffects.TownSecurityChange(() => MobileParty.MainParty.CurrentSettlement.Town, -10)
		});
		incident91.AddOption("{=k4aNfNHg}As on the battlefield, so on the workshop floor. Let each team work at its own pace, receiving bonuses for output, but also taking responsibiliy for injuries.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, 100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -200),
			CampaignIncidentEffects.WorkshopProfitabilityChange(GetOccupationalSafetyWorkshop, 0.05f)
		});
		...
		{
			Workshop handInTheStrongboxWorkshop = GetHandInTheStrongboxWorkshop();
			if (handInTheStrongboxWorkshop == null)
			{
				return false;
			}
		...
		incident92.AddOption("{=47hUs9yg}You make an example of the foreman. You hand him over to the town authorities and encourage them to apply the full penalty - branding as a thief and exile from the town.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, 100),
			CampaignIncidentEffects.WorkshopProfitabilityChange(GetHandInTheStrongboxWorkshop, -0.1f).WithChance(0.3f)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 100),
			CampaignIncidentEffects.WorkshopProfitabilityChange(GetHandInTheStrongboxWorkshop, -0.05f)
		});
		...
		incident92.AddOption("{=G8eaX10B}You treat all your workers as potential thieves, having them searched coming and going from the workshop.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100) });
		Incident incident93 = RegisterIncident("incident_jobs_for_the_lads", "{=0avcvATJ}Jobs for the Lads", "{=cJdoDJ4M}One prominent resident of {TOWN}, {ARTISAN_NAME}, approaches you. Your {WORKSHOP} has been hiring workers who have recently migrated in from the countryside. He requests that you fire them and take on some local boys from the back alleys. If you refuse, he won't be responsible if the carts carrying your shipments get swarmed by traffic going through tight gates, slowing down production.", "EnteringTown", "Workshop", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			Workshop jobsForTheLadsWorkshop = GetJobsForTheLadsWorkshop();
			if (jobsForTheLadsWorkshop == null)
			{
				return false;
			}
			Hero jobsForTheLadsArtisan3 = GetJobsForTheLadsArtisan();
			if (jobsForTheLadsArtisan3 == null)
			{
				return false;
			}
			description.SetTextVariable("TOWN", jobsForTheLadsWorkshop.Settlement.Name);
			description.SetTextVariable("WORKSHOP", jobsForTheLadsWorkshop.Name);
			description.SetTextVariable("ARTISAN_NAME", jobsForTheLadsArtisan3.Name);
			return true;
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100),
			CampaignIncidentEffects.HeroRelationChange(GetJobsForTheLadsArtisan, 10)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.WorkshopProfitabilityChange(GetJobsForTheLadsWorkshop, -0.05f)
		}, delegate(TextObject text)
		{
			Hero jobsForTheLadsArtisan2 = GetJobsForTheLadsArtisan();
			text.SetTextVariable("ARTISAN_NAME", jobsForTheLadsArtisan2.Name);
			return true;
		});
		...
		{
			Hero jobsForTheLadsArtisan = GetJobsForTheLadsArtisan();
			text.SetTextVariable("ARTISAN_NAME", jobsForTheLadsArtisan.Name);
			return true;
		});
		Incident incident94 = RegisterIncident("incident_respect_for_the_slain", "{=EilA5dlq}Respect for the Slain", "{=pNVKWAKo}Throughout your visit to {VILLAGE}, one of your older {TROOP_TYPE} seems moody and downcast. As you march out, he suddenly stops, stares at a roadside stall, then breaks ranks from your party and starts shouting at the vendor. The story quickly comes tumbling out. He served in one of the Empire's old legions in a battle here against the {NEAREST_OTHER_CULTURE}, and the vendor is selling scraps of metal that were clearly looted from the graves of the dead.", "LeavingVillage", "TroopSettlementRelation", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			CharacterObject respectForTheSlainSelectedTroop = GetRespectForTheSlainSelectedTroop();
			if (respectForTheSlainSelectedTroop == null)
			{
				return false;
			}
			CultureObject respectForTheSlainNearestOtherCulture = GetRespectForTheSlainNearestOtherCulture();
			if (respectForTheSlainNearestOtherCulture == null)
			{
				return false;
			}
			description.SetTextVariable("VILLAGE", MobileParty.MainParty.LastVisitedSettlement.Name);
			description.SetTextVariable("TROOP_TYPE", respectForTheSlainSelectedTroop.Name);
			description.SetTextVariable("NEAREST_OTHER_CULTURE", respectForTheSlainNearestOtherCulture.Name);
			return true;
		});
		...
		incident94.AddOption("{=WGmKib15}Erupt in fury at the vendor and accuse him of grave desecration. It's unlikely that the thefts will stop, but your men will be pleased.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 50),
			CampaignIncidentEffects.RenownChange(2f),
			CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, -5)
		});
		...
		incident95.AddOption("{=Wenvx1OE}Reach into your treasury and grab great fistfuls of silver denars, hurling it to your men and praising them by name for their deeds.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Calculating, -100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 200),
			CampaignIncidentEffects.GoldChange(() => -(int)((float)Hero.MainHero.Gold * 0.5f)),
			CampaignIncidentEffects.RenownChange(5f)
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.GoldChange(() => MobileParty.MainParty.TotalWage * -7)
		});
		incident95.AddOption("{=WnlD7awi}Gruffly tell them that you were pleased they did their duty.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100) });
		Incident incident96 = RegisterIncident("incident_letter_of_reference", "{=q0HIXEal}Letter of Reference", "{=D7ZcX8XR}As you pass through the market, {NOTABLE.NAME} requests a minute of your time. {?NOTABLE.GENDER}Her{?}His{\\?} cousin is planning on doing some business in {TOWN}, and {?NOTABLE.GENDER}she{?}he{\\?} would like a letter of reference from you. You know next to nothing about the lad, although {NOTABLE.NAME} swears that he is honest and conscientious. You find you are receiving a number of these types of requests as your fame spreads.", "LeavingTown", "Profit", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			Hero letterOfReferenceNotable3 = GetLetterOfReferenceNotable();
			if (letterOfReferenceNotable3 == null || Hero.MainHero.Clan.Renown < 200f)
			{
				return false;
			}
			description.SetCharacterProperties("NOTABLE", letterOfReferenceNotable3.CharacterObject);
			description.SetTextVariable("TOWN", letterOfReferenceNotable3.CurrentSettlement.Name);
			return true;
		});
		incident96.AddOption("{=z15Ux7sh}Write truthfully that you don't know much about the bearer of the letter, but he comes from good family.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, 100) });
		incident96.AddOption("{=aZwaFhA6}Write effusively about the lad's qualities.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, 100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, -100),
			CampaignIncidentEffects.HeroRelationChange(GetLetterOfReferenceNotable, 20)
		});
		...
		{
			Hero letterOfReferenceNotable2 = GetLetterOfReferenceNotable();
			text.SetCharacterProperties("NOTABLE", letterOfReferenceNotable2.CharacterObject);
			return true;
		});
		...
		{
			CampaignIncidentEffects.ChangeItemAmount(GetTillerAndWandererHorseItem, () => 3),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, -100),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, 10),
			CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, 20),
			CampaignIncidentEffects.TownSecurityChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, -10),
			CampaignIncidentEffects.Custom(null, delegate
			{
				Clan tillerAndWandererBanditPartyClan = GetTillerAndWandererBanditPartyClan();
				for (int i = 0; i < 3; i++)
				{
					...
					MobileParty mobileParty = BanditPartyComponent.CreateBanditParty("incident_tiller_and_wanderer_bandit_revenge_" + i, tillerAndWandererBanditPartyClan, hideout, isBossParty: false, null, initialPosition);
					mobileParty.MemberRoster.AddToCounts(tillerAndWandererBanditPartyClan.Culture.BanditBandit, MobileParty.MainParty.RandomIntWithSeed((uint)Campaign.Current.IncidentManager.ActiveIncidentSeed, 5, 17));
			...
			}, (IncidentEffect effect) => new IncidentHint(new TextObject("{=ayfKQixq}3 bandit parties spawn nearby")))
		});
		...
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, 100),
			CampaignIncidentEffects.TownSecurityChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, -5),
			CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, 10)
		});
		incident97.AddOption("{=fkQj3TIP}Tell the nomads that proud warriors such as themselves need not worry what these dirt-diggers think. Share a meal with them, and see if some will join you.", new List<IncidentEffect>
		{
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Valor, 100),
			CampaignIncidentEffects.ChangeTroopAmount(GetTillerAndWandererTroop, 4),
			CampaignIncidentEffects.SettlementRelationChange(() => MobileParty.MainParty.LastVisitedSettlement, -10),
			CampaignIncidentEffects.TownProsperityChange(() => MobileParty.MainParty.LastVisitedSettlement.Town, -10)
		});
		Incident incident98 = RegisterIncident("incident_cries_in_the_mist", "{=ELXomm1C}Cries in the Mist", "{=SAXnFQLq}The people of {VILLAGE_NAME} warned you to take care outside their village. When the mist gathers, The woods will be haunted by spirits whose shriek foretells death. Now the gloom is gathering, and you hear mutterings from your men. You are certain that they will soon be hearing wailing and seeing apparitions, real or imagined. The horses are becoming twitchy too, picking up on the mood of the men, and accidents are likely to happen.", "LeavingVillage", "DreamsSongsAndSigns", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			for (float num3 = -3f; num3 <= 3f; num3 += 0.25f)
			{
				for (float num4 = -3f; num4 <= 3f; num4 += 0.25f)
				{
					...
					{
						CampaignVec2 campaignVec = new CampaignVec2(village.Settlement.Position.ToVec2() + new Vec2(num3, num4), isOnLand: true);
		...
		incident98.AddOption("{=uD05twgi}Tell your men to ignore the superstitions of villagers, and press on.", new List<IncidentEffect>
		{
			...
			CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100),
			CampaignIncidentEffects.MoraleChange(-20f),
			CampaignIncidentEffects.WoundTroopsRandomly((TroopRosterElement x) => x.Character.IsMounted, () => Math.Min(MobileParty.MainParty.RandomIntWithSeed((uint)Campaign.Current.IncidentManager.ActiveIncidentSeed, 1, 6), (from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
		...
		Incident incident99 = RegisterIncident("incident_feeling_the_bite", "{=0yKJ0J0N}Feeling the Bite", "{=D9T64ejr}As you leave {VILLAGE_NAME}, several of the villagers approach you. They warn you that stagnant ponds from the last rain have yet to fully dry and have bred mosquitos. They offer to sell you a concoction of oils including rosemary, catmint and other herbs that they say will ward off all flying vermin.", "LeavingVillage", "HardTravel", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			if (MobileParty.MainParty.LastVisitedSettlement != null)
			{
				...
				bool result3 = getSeasonOfYear == CampaignTime.Seasons.Spring || getSeasonOfYear == CampaignTime.Seasons.Summer;
				description.SetTextVariable("VILLAGE_NAME", MobileParty.MainParty.LastVisitedSettlement.Name);
				return result3;
			}
			return false;
		...
		incident99.AddOption("{=qCoqaa70}Have your men wrap themselves up as best they can and press on.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -50) });
		...
		Incident incident100 = RegisterIncident("incident_nearly_a_gift_horse", "{=N19m9qmd}Nearly a Gift Horse", "{=WR7NCetE}As you leave {VILLAGE_NAME}, a scraggly young man waves to attract your attention. He leads a magnificent {RARE_HORSE}, and says that he will sell it to you for a mere 500 denars. He claims he found it wandering on a heath, almost dead from exhaustion and neglect, and he nursed it back to health. He won't go into town because he says he would be unfairly accused of theft.", "LeavingVillage", "TroopSettlementRelation", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			ItemObject nearlyAGiftHorseHorse = GetNearlyAGiftHorseHorse();
			if (nearlyAGiftHorseHorse == null)
			{
				return false;
			}
			description.SetTextVariable("VILLAGE_NAME", MobileParty.MainParty.LastVisitedSettlement.Name);
			description.SetTextVariable("RARE_HORSE", nearlyAGiftHorseHorse.Name);
		...
		incident100.AddOption("{=YVETzHXK}Plausible enough, and you have no time to check the provenance of every bargain you find. Buy the horse!", new List<IncidentEffect>
		{
			CampaignIncidentEffects.GoldChange(() => -500),
			CampaignIncidentEffects.TraitChange(DefaultTraits.Honor, -50),
			CampaignIncidentEffects.ChangeItemAmount(GetNearlyAGiftHorseHorse, () => 1)
		});
		...
		incident100.AddOption("{=CyM5bAdB}You're not sure you trust the seller, but you're not handing over to officials who might just hang him and seize the horse themselves. Go on your way.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Mercy, 100) });
		...
		Incident incident101 = RegisterIncident("incident_hammer_of_the_sun", "{=R80p9Ywt}Hammer of the Sun", "{=th5MgyAZ}As you enter {TOWN_NAME}, one of your {ASERAI_TROOPS} approaches you. Many of his comrades are not used to the heat of the desert, he says. They march after drinking wine through the evening but are not careful to properly take water throughout the day, as the sun beats down on them. He has seen men collapse and die from such carelessness and ignorance.", "EnteringTown", "HardTravel", CampaignTime.Years(1000f), delegate(TextObject description)
		{
			...
			CharacterObject character = IncidentHelper.GetSeededRandomElement(troopRoster.Where((TroopRosterElement x) => x.Character.Culture.StringId == "aserai" && !x.Character.IsHero).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
			if (character == null)
			{
				return false;
			}
		...
		incident101.AddOption("{=3Uub2e17}Tell your men that there shall be no wine tonight, and none henceforth until they learn how to handle themselves in the desert.", new List<IncidentEffect> { CampaignIncidentEffects.TraitChange(DefaultTraits.Generosity, -100) });
		...
		static ItemObject GetBrokenWagonCheapestHorseItem()
		{
			List<ItemRosterElement> list6 = MobileParty.MainParty.ItemRoster.Where((ItemRosterElement x) => x.EquipmentElement.Item.HasHorseComponent).ToList();
			return list6[MobileParty.MainParty.RandomIntWithSeed((uint)Campaign.Current.IncidentManager.ActiveIncidentSeed, list6.Count)].EquipmentElement.Item;
		}
		static ItemObject GetBrokenWagonDraughtAnimalItem()
		{
			List<ItemRosterElement> list7 = MobileParty.MainParty.ItemRoster.Where((ItemRosterElement x) => x.EquipmentElement.Item.HasHorseComponent || x.EquipmentElement.Item.StringId == "mule" || x.EquipmentElement.Item.StringId == "mule_unmountable").ToList();
			return list7[MobileParty.MainParty.RandomIntWithSeed((uint)Campaign.Current.IncidentManager.ActiveIncidentSeed, list7.Count)].EquipmentElement.Item;
		}
		static ItemObject GetDesperateTimesFoodItem()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.ItemRoster.Where((ItemRosterElement x) => x.EquipmentElement.Item.IsFood && x.Amount >= 1).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).EquipmentElement.Item;
		}
		...
		static CharacterObject GetFirstNobleTroop()
		{
			return IncidentHelper.GetSeededRandomElement((from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
				where x.Character.Tier >= 4 && x.Character.HasMount() && !x.Character.IsHero
				select x).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
		}
		static CharacterObject GetGnosticBoysNobleTroop()
		{
			return IncidentHelper.GetSeededRandomElement((from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
				where !x.Character.IsHero && x.Character.Culture.StringId == "empire" && x.Character.Tier >= 3 && x.Number >= 2
				select x).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
		}
		static Workshop GetHandInTheStrongboxWorkshop()
		{
			Town seededRandomElement = IncidentHelper.GetSeededRandomElement(Town.AllTowns.Where((Town x) => x.Workshops.Any((Workshop y) => y.Owner == Hero.MainHero)).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
			if (seededRandomElement == null)
			{
				return null;
			}
			return IncidentHelper.GetSeededRandomElement(seededRandomElement.Workshops.Where((Workshop w) => w.Owner == Hero.MainHero).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static TroopRosterElement GetHonorTheSlainFoeRandomTroop()
		{
			return IncidentHelper.GetSeededRandomElement((from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
				where !x.Character.IsHero
				select x).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		...
		static CharacterObject GetInsultCavalryman()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.MemberRoster.GetTroopRoster().Where(CavalryTroopsPredicate).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
		}
		static CharacterObject GetInsultFootman()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.MemberRoster.GetTroopRoster().Where(FootmenTroopsPredicate).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
		}
		static Hero GetIntriguingRumorsRivalLord()
		{
			return IncidentHelper.GetSeededRandomElement(Hero.MainHero.Clan.Kingdom?.AliveLords.Where((Hero lord) => lord != Hero.MainHero && Hero.MainHero.GetRelation(lord) < 0).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static CharacterObject GetJobOfferTroop()
		{
			return IncidentHelper.GetSeededRandomElement((from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
				where !x.Character.IsHero && x.Character.Tier >= 5
				select x).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
		}
		static Hero GetJobsForTheLadsArtisan()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.CurrentSettlement.Town.Settlement.Notables.Where((Hero n) => n.GetTraitLevel(DefaultTraits.Honor) <= 0 && Hero.MainHero.GetRelation(n) < 10 && n.Occupation == Occupation.Artisan).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static Workshop GetJobsForTheLadsWorkshop()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.CurrentSettlement.Town.Workshops.Where((Workshop w) => w.Owner == Hero.MainHero).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static Hero GetLetterOfReferenceNotable()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.LastVisitedSettlement.Town.Settlement.Notables.Where((Hero x) => x.Occupation == Occupation.Merchant && x.GetTraitLevel(DefaultTraits.Honor) <= 0).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static CharacterObject GetMadDogTroop()
		{
		...
		static Hero GetMarketManipulationMerchant()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.CurrentSettlement.Notables.Where((Hero x) => x.Occupation == Occupation.Merchant).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static Village GetMisplacedVengeanceLootedVillage()
		{
			Village result2 = null;
			...
			foreach (Village item6 in Village.All)
			{
				...
				{
					float distance2 = Campaign.Current.Models.MapDistanceModel.GetDistance(MobileParty.MainParty.LastVisitedSettlement.Village.Settlement, item6.Settlement, isFromPort: false, isTargetingPort: false, MobileParty.NavigationType.All);
					if (distance2 <= num2 && distance2 <= 1000f)
					{
						result2 = item6;
						num2 = distance2;
					}
		...
		static ItemObject GetNearlyAGiftHorseHorse()
		{
			return IncidentHelper.GetSeededRandomElement(Campaign.Current.AllItems.Where((ItemObject x) => x.ItemCategory == DefaultItemCategories.Horse && x.Culture == MobileParty.MainParty.LastVisitedSettlement.Culture && x.Value > 500).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static CharacterObject GetNoMoodForMercyRandomTroop()
		{
			return IncidentHelper.GetSeededRandomElement((from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
				where !x.Character.IsHero
				select x).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
		}
		static CharacterObject GetNobleTroop(CultureObject culture)
		{
			List<TroopRosterElement> list3 = (from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
			...
			return IncidentHelper.GetSeededRandomElement(list3, Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
		}
		static CharacterObject GetNobleTroopForUpgrade()
		{
			CharacterObject regularTroop = GetRegularTroop();
			if (regularTroop == null)
			{
				return null;
			}
			return GetNobleTroop(regularTroop.Culture);
		}
		static Workshop GetOccupationalSafetyWorkshop()
		{
			WorkshopType breweryWorkshopType = WorkshopType.Find("brewery");
			WorkshopType smithyWorkshopType = WorkshopType.Find("smithy");
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.CurrentSettlement.Town.Workshops.Where((Workshop w) => w.Owner == Hero.MainHero && (w.WorkshopType == breweryWorkshopType || w.WorkshopType == smithyWorkshopType)).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static Village GetOtherVillage()
		{
			MBReadOnlyList<Village> villages = MobileParty.MainParty.LastVisitedSettlement.Town.Villages;
			Village villageOne = GetVillageOne();
			Village result = null;
			...
			foreach (Village item7 in Village.All)
			{
				...
				{
					float distance = Campaign.Current.Models.MapDistanceModel.GetDistance(villageOne.Settlement, item7.Settlement, isFromPort: false, isTargetingPort: false, MobileParty.NavigationType.Default);
		...
		static ItemObject GetPurebredHorseItem()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.ItemRoster.Where((ItemRosterElement x) => x.EquipmentElement.Item.HasHorseComponent && x.Amount >= 1).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).EquipmentElement.Item;
		}
		static TroopRosterElement GetRandomCavalryTroop()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.MemberRoster.GetTroopRoster().Where(CavalryTroopPredicate).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static TroopRosterElement GetRandomTroop()
		{
			return IncidentHelper.GetSeededRandomElement((from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
				where !x.Character.IsHero
				select x).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static CharacterObject GetRegularTroop()
		{
			List<TroopRosterElement> list4 = (from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
			...
			return IncidentHelper.GetSeededRandomElement(list4, Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
		}
		...
		static CharacterObject GetRespectForTheSlainSelectedTroop()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.MemberRoster.GetTroopRoster().Where(HighTierTroopsPredicate).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
		}
		static Workshop GetSecessionOfThePlebsWorkshop()
		{
			...
			return IncidentHelper.GetSeededRandomElement(IncidentHelper.GetSeededRandomElement(list, Campaign.Current.IncidentManager.ActiveIncidentSeed).Workshops.Where((Workshop w) => w.Owner == Hero.MainHero).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static CharacterObject GetSecondNobleTroop()
		{
			return IncidentHelper.GetSeededRandomElement((from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
		...
		static CharacterObject GetSleepingSentryTroop()
		{
			List<TroopRosterElement> list5 = (from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
		...
		static ItemObject GetSpeculativeInvestmentOtherGood()
		{
			...
			}, Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static ItemObject GetSpoiledFoodItem()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.ItemRoster.Where((ItemRosterElement x) => x.EquipmentElement.Item.IsFood && x.Amount >= 2).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).EquipmentElement.Item;
		}
		static CharacterObject GetTheQuietLifeTroop()
		{
			return IncidentHelper.GetSeededRandomElement((from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
				where !x.Character.IsHero && x.Character.Tier >= 4 && x.Number >= 3
				select x).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
		}
		static Clan GetTillerAndWandererBanditPartyClan()
		{
		...
		static ItemRosterElement GetTradeProposalGood()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.ItemRoster.Where((ItemRosterElement x) => x.EquipmentElement.Item.IsTradeGood).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		...
		static ItemObject GetVeteranFoodItem()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.ItemRoster.Where((ItemRosterElement x) => x.EquipmentElement.Item.IsFood && x.Amount >= 1).ToList(), Campaign.Current.IncidentManager.ActiveIncidentSeed).EquipmentElement.Item;
		}
		static CharacterObject GetVeteranMaxTierTroop()
		{
			List<TroopRosterElement> list9 = (from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
				where !x.Character.IsHero
				select x).ToList();
			if (list9.Count == 0)
			{
			...
			return TaleWorlds.Core.Extensions.MaxBy(list9, (TroopRosterElement x) => x.Character.Tier).Character;
		}
		static Village GetVillageOne()
		{
			return IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.LastVisitedSettlement.Town.Villages, Campaign.Current.IncidentManager.ActiveIncidentSeed);
		}
		static Building GetWagesOfWarAndDiseasesBuilding()
		{
			return MobileParty.MainParty.LastVisitedSettlement.Town.Buildings.FirstOrDefault((Building x) => x.BuildingType == DefaultBuildingTypes.CastleFarmlands);
		}
		static CharacterObject GetWantedCriminalTroop()
		{
			List<TroopRosterElement> list8 = (from x in MobileParty.MainParty.MemberRoster.GetTroopRoster()
				where !x.Character.IsHero
				select x).ToList();
			if (list8.Count == 0)
			{
			...
			return IncidentHelper.GetSeededRandomElement(list8, Campaign.Current.IncidentManager.ActiveIncidentSeed).Character;
		}
