...
public class IncidentsCampaignBehaviour : CampaignBehaviorBase, INonReadyObjectHandler
{
	...
	private void InitializeIncidents()
	{
		...
		{
			IncidentEffect.DisorganizeParty(),
			IncidentEffect.DemoteTroopsRandomlyWithPredicate(TroopToDemotePredicate, TroopToDemoteToPredicate, 1).WithChance(0.5f)
		});
		...
		{
			IncidentEffect.TraitChange(DefaultTraits.Mercy, 100),
			IncidentEffect.DemoteTroopsRandomlyWithPredicate(TroopToDemotePredicate, TroopToDemoteToPredicate, 1)
		});
		...
		{
			Village seededRandomElement2 = IncidentHelper.GetSeededRandomElement(Village.All.Where((Village x) => x.VillageType == DefaultVillageTypes.VineYard).ToList(), _activeIncidentSeed);
		...
		incident42.AddOption("{=SfO3uwdX}Take a brief walk to clear your head, and when you come back to find the prisoners gone, do not ask questions", new List<IncidentEffect>
		{
			...
			}, delegate
			{
				...
				return new List<TextObject> { new TextObject("{=gejYla80}Lose {AMOUNT} {?AMOUNT > 1}prisoners{?}prisoner{\\?} of {CULTURE} culture").SetTextVariable("AMOUNT", 5).SetTextVariable("CULTURE", misplacedVengeanceLootedVillage?.Settlement.Culture.Name) };
			})
		});
		...
		{
			CharacterObject character3 = IncidentHelper.GetSeededRandomElement(MobileParty.MainParty.MemberRoster.GetTroopRoster().Where(LoveMarriageTroopPredicate).ToList(), _activeIncidentSeed).Character;
			if (character3 == null)
			{
				return false;
			}
		...
		incident97.AddOption("{=fI8Bd468}Seize the nomads' herds to compensate you and your tenants for their crops.", new List<IncidentEffect>
		{
			...
			IncidentEffect.Custom(null, delegate
			{
				...
				for (int i = 0; i < 3; i++)
				{
					...
					mobileParty.MemberRoster.AddToCounts(tillerAndWandererBanditPartyClan.Culture.BanditBandit, MobileParty.MainParty.RandomIntWithSeed((uint)_activeIncidentSeed, 5, 17));
		...
		ItemObject GetBrokenWagonCheapestHorseItem()
		{
			...
			return list6[MobileParty.MainParty.RandomIntWithSeed((uint)_activeIncidentSeed, list6.Count)].EquipmentElement.Item;
		}
		ItemObject GetBrokenWagonDraughtAnimalItem()
		{
			...
			return list7[MobileParty.MainParty.RandomIntWithSeed((uint)_activeIncidentSeed, list7.Count)].EquipmentElement.Item;
		}
		...
		CharacterObject GetMadDogTroop()
		{
			...
			return IncidentHelper.GetSeededRandomElement(list2, _activeIncidentSeed);
		}
		...
		CharacterObject GetNobleTroop(CultureObject culture)
		{
			...
			return IncidentHelper.GetSeededRandomElement(list3, _activeIncidentSeed).Character;
		}
		...
		CharacterObject GetRegularTroop()
		{
			...
			return IncidentHelper.GetSeededRandomElement(list4, _activeIncidentSeed).Character;
		}
		...
		CharacterObject GetSleepingSentryTroop()
		{
			...
			return IncidentHelper.GetSeededRandomElement(list5, _activeIncidentSeed).Character;
		}
		...
		CharacterObject GetWantedCriminalTroop()
		{
			...
			return IncidentHelper.GetSeededRandomElement(list8, _activeIncidentSeed).Character;
		}
