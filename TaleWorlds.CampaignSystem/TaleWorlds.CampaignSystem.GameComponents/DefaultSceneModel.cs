...
public class DefaultSceneModel : SceneModel
{
	...
	public override string GetBattleSceneForMapPatch(MapPatchData mapPatch, bool isNavalEncounter)
	{
		...
		if (mBList.IsEmpty())
		{
			...
			CampaignVec2 vec = MobileParty.MainParty.Position;
			mapSceneWrapper.GetEnvironmentTerrainTypesCount(in vec, out var currentPositionTerrainType2);
			mBList = GameSceneDataManager.Instance.SingleplayerBattleScenes.Where((SingleplayerBattleSceneData scene) => scene.Terrain == currentPositionTerrainType2 && scene.IsNaval == isNavalEncounter).ToMBList();
			if (mBList.IsEmpty())
			{
		...
		if (mBList.Count > 1)
		{
			if (isNavalEncounter)
			{
				...
				CampaignVec2 vec = MobileParty.MainParty.Position;
				mapSceneWrapper2.GetEnvironmentTerrainTypesCount(in vec, out var currentPositionTerrainType);
				List<SingleplayerBattleSceneData> list = mBList.Where((SingleplayerBattleSceneData scene) => scene.Terrain == currentPositionTerrainType).ToList();
				if (!list.IsEmpty())
				{
