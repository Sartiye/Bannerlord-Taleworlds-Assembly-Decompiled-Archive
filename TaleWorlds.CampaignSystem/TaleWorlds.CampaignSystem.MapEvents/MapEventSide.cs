...
public class MapEventSide
{
	...
	internal void HandleMapEventEndForPartyInternal(PartyBase party)
	{
		...
		if (party.IsMobile && party != PartyBase.MainParty && party.IsActive && (party.NumberOfAllMembers == 0 || (!flag && !MapEvent.EndedByRetreat && (party.NumberOfHealthyMembers == 0 || (_mapEvent.BattleState != 0 && party.MobileParty.IsMilitia)) && (party.MobileParty.Army == null || party.MobileParty.Army.LeaderParty.Party.NumberOfHealthyMembers == 0))) && (!party.MobileParty.IsDisbanding || party.MemberRoster.Count == 0))
		{
	...
	private void MakeReady(bool includeHumanPlayers, int sizeOfSide, FlattenedTroopRoster priorTroops = null)
	{
		...
		for (int i = 0; i < _battleParties.Count; i++)
		{
			...
			int a2 = TaleWorlds.Library.MathF.Round((float)mapEventParty.HealthyManCountAtStart / (float)healthyTroopCountAtMapEventStart * (float)sizeOfSide);
			int num2;
			if (i < _battleParties.Count - 1)
			{
				num2 = TaleWorlds.Library.MathF.Min(a2, sizeOfSide - num);
				num += num2;
			}
			else
			{
