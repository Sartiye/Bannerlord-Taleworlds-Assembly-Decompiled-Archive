...
public class CharacterData
{
	...
	private static void InitializeHeroFromCharacterData(Hero target, CharacterData characterData)
	{
		...
		for (int j = 0; j < characterData.SkillsArray.Length; j++)
		{
			...
			if (object2 != null)
			{
				int focus2 = target.HeroDeveloper.GetFocus(object2);
				...
				target.HeroDeveloper.SetSkillXp(object2, num2);
				target.SetSkillValue(object2, value);
				focus = Math.Max(Math.Min(focus, Campaign.Current.Models.CharacterDevelopmentModel.MaxFocusPerSkill), 0);
				if (focus2 < focus)
				{
					target.HeroDeveloper.AddFocus(object2, focus - focus2, checkUnspentFocusPoints: false);
				}
				else
				{
					target.HeroDeveloper.RemoveFocus(object2, focus2 - focus);
				}
			}
		}
		for (int k = 0; k < characterData.Traits.Length; k++)
		{
			...
			if (object3 != null)
			{
				value2 = Math.Max(Math.Min(value2, object3.MaxValue), object3.MinValue);
				target.SetTraitLevel(object3, value2);
			}
		}
		for (int l = 0; l < characterData.AttributesArray.Length; l++)
		{
			...
			if (object4 != null)
			{
				int changeAmount = ((target.GetAttributeValue(object4) > value3) ? (value3 - target.GetAttributeValue(object4)) : (value3 - target.GetAttributeValue(object4)));
				target.HeroDeveloper.AddAttribute(object4, changeAmount, checkUnspentPoints: false);
			}
		}
		target.ClearPerks();
