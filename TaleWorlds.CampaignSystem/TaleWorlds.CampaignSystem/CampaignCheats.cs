	...
	public static string AddItemToPlayerParty(List<string> strings)
	{
		...
		if (@object != null)
		{
			...
			if (separatedNames.Count == 1)
			{
				...
				return string.Concat(@object.Name, " has been given to the main party.");
			}
			ItemModifier object2 = Game.Current.ObjectManager.GetObject<ItemModifier>(separatedNames[1]);
			if (object2 != null)
			{
				EquipmentElement rosterElement = new EquipmentElement(@object, object2);
			...
			if (int.TryParse(separatedNames[1], out result) && result >= 1)
			{
				...
				MobileParty.MainParty.ItemRoster.AddToCounts(@object, result);
				return string.Concat(@object.Name, " has been given to the main party.");
			}
		...
		if (!string.IsNullOrEmpty(requestedId))
		{
			...
			if (mBReadOnlyList != null && mBReadOnlyList.Any())
			{
				for (int i = 0; i <= 7; i++)
				{
					...
					{
						if (func(requestedId, mBReadOnlyList[j].StringId, (CheatTextControl)i))
						{
					...
					{
						if (func(requestedId, mBReadOnlyList[k].GetName().ToString(), (CheatTextControl)i) && !mBList.ContainsQ(mBReadOnlyList[k]))
						{
			...
			else
			{
				...
				{
					T val = mBList[l];
					errorMessage += $"\nObject {l + 1}: {val.GetName()}  Id:  {val.StringId}";
				}
			}
			return false;
		}
