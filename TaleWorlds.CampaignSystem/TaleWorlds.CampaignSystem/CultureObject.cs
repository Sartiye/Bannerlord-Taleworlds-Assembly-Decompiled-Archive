...
public sealed class CultureObject : BasicCultureObject
{
	...
	public override void Deserialize(MBObjectManager objectManager, XmlNode node)
	{
		...
		foreach (XmlNode item4 in childNodes)
		{
			...
			else if (item4.Name == "notable_templates")
			{
			...
			else if (item4.Name == "banner_bearer_replacement_weapons")
			{
			...
			else if (item4.Name == "caravan_party_templates")
			{
				foreach (XmlNode childNode16 in item4.ChildNodes)
				{
					mBList10.Add(objectManager.ReadObjectReferenceFromXml<PartyTemplateObject>("id", childNode16));
				}
			}
			else if (item4.Name == "elite_caravan_party_templates")
			{
				foreach (XmlNode childNode17 in item4.ChildNodes)
				{
			...
			{
				if (!(item4.Name == "available_ship_hulls"))
				{
					continue;
				}
				foreach (XmlNode childNode18 in item4.ChildNodes)
				{
