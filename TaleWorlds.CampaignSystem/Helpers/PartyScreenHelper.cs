...
public static class PartyScreenHelper
{
	...
	public static void OpenScreenAsManageTroops(MobileParty leftParty)
	{
		...
		{
			reason = new TextObject("{=SfpoMma5}You can't talk to your companions while managing the garrison.");
			return false;
		};
		partyState.PartyScreenLogic = partyScreenLogic;
		Game.Current.GameStateManager.PushState(partyState);
	...
	public static void OpenScreenForManagingAlley(bool isNewAlley, TroopRoster memberRosterLeft, IsTroopTransferableDelegate isTroopTransferable, PartyPresentationDoneButtonConditionDelegate doneButtonCondition, PartyPresentationDoneButtonDelegate onDoneClicked, TextObject leftPartyName, PartyPresentationCancelButtonDelegate onCancelButtonClicked)
	{
		...
		partyScreenLogicInitializationData.PartyPresentationDoneButtonDelegate = onDoneClicked;
		partyScreenLogicInitializationData.LeftMemberRoster = memberRosterLeft;
		partyScreenLogicInitializationData.LeftPrisonerRoster = TroopRoster.CreateDummyTroopRoster();
		partyScreenLogicInitializationData.PartyPresentationCancelButtonDelegate = onCancelButtonClicked;
		partyScreenLogicInitializationData.RightMemberRoster = troopRoster;
		partyScreenLogicInitializationData.RightPrisonerRoster = TroopRoster.CreateDummyTroopRoster();
		partyScreenLogicInitializationData.LeftPartyMembersSizeLimit = Campaign.Current.Models.AlleyModel.MaximumTroopCountInPlayerOwnedAlley + 1;
		partyScreenLogicInitializationData.RightPartyMembersSizeLimit = PartyBase.MainParty.PartySizeLimit;
		partyScreenLogicInitializationData.RightPartyPrisonersSizeLimit = PartyBase.MainParty.PrisonerSizeLimit;
		...
		partyScreenLogicInitializationData.AccompanyingTransferState = PartyScreenLogic.TransferState.NotTransferable;
		partyScreenLogicInitializationData.PartyScreenMode = PartyScreenMode.TroopsManage;
		partyScreenLogicInitializationData.IsTroopUpgradesDisabled = true;
		partyScreenLogicInitializationData.ShowProgressBar = true;
		partyScreenLogicInitializationData.TransferHealthiesGetWoundedsFirst = false;
		partyScreenLogicInitializationData.IsDismissMode = false;
		partyScreenLogicInitializationData.QuestModeWageDaysMultiplier = 0;
		partyScreenLogicInitializationData.Header = null;
		partyScreenLogicInitializationData.RightOwnerParty = PartyBase.MainParty;
		partyScreenLogicInitializationData.RightPartyName = PartyBase.MainParty.Name;
	...
	public static void OpenScreenWithDummyRoster(TroopRoster leftMemberRoster, TroopRoster leftPrisonerRoster, TroopRoster rightMemberRoster, TroopRoster rightPrisonerRoster, TextObject leftPartyName, TextObject rightPartyName, int leftPartySizeLimit, int rightPartySizeLimit, PartyPresentationDoneButtonConditionDelegate doneButtonCondition, PartyScreenClosedDelegate onPartyScreenClosed, IsTroopTransferableDelegate isTroopTransferable, CanTalkToHeroDelegate canTalkToTroopDelegate = null, PartyPresentationCancelButtonActivateDelegate partyPresentationCancelButtonActivateDelegate = null)
	{
		...
		partyScreenLogicInitializationData.TroopTransferableDelegate = isTroopTransferable;
		partyScreenLogicInitializationData.CanTalkToTroopDelegate = canTalkToTroopDelegate ?? new CanTalkToHeroDelegate(CanTalkToHero);
